...
if ( class_exists( 'CX_Email_Cart_Plugin_Update_Checker' ) ) return;

class CX_Email_Cart_Plugin_Update_Checker {
			...
			if ( isset( $_GET['plugin'] ) && $this->slug == $_GET['plugin'] ) {
				wp_localize_script(
					...
					array(
						...
						'update_button_text' => __( 'Please register on the Plugins page to get this update', 'email-cart' ),
			...
			<div class="cx-plugin-update-enter-code notice notice-success is-dismissibleZZ">
				<form method="post">
					...
					<?php if ( $notification ) { ?>
						...
						<?php if ( 'wrong-format' == $notification ) { ?>
							...
							<div class="cx-plugin-update-notification">
								<?php _e( 'That is not a valid purchase code format. It should look like: <code>f128a5a1-5a1c-4e4e-82ec-a12b3c4d5e6f</code>.', 'email-cart' ); ?>
							</div>
							
						<?php } else if ( 'connection-error' == $notification ) { ?>
							
							<div class="cx-plugin-update-notification">
								<?php _e( 'You are unable to connect to our update server at this time. Please check your connection, or try again later.', 'email-cart' ); ?>
							</div>
						
						<?php } else if ( 'invalid-purchase-code' == $notification ) { ?>
							
							<div class="cx-plugin-update-notification">
								<?php _e( "It doesn't seem like you have purchased our plugin yet. Please <a href='#'>purchase it from CodeCanyon</a> to get access to support and updates.", 'email-cart' ); ?>
							</div>
						
						<?php } else if ( 'success' == $notification ) { ?>
							
							<div class="cx-plugin-update-notification cx-plugin-update-notification-success">
								<?php _e( "Thanks. You're registered and can now receive updates.", 'email-cart' ); ?>
							</div>
					...
					<div class="ec-plugin-update-row">
						<label><?php _e( 'Purchase Code', 'email-cart' ); ?></label> 
						...
						<input type="submit" class="button" value="<?php _e( 'Save', 'email-cart' ); ?>" >
					</div>
	...
	public function plugin_row_meta( $links, $file ) {
		...
		if ( $file == plugin_basename( $this->pluginFile ) ) {
			...
			if ( ! $purchase_code ) {
				...
				$row_meta = array(
					'support' => '<a class="cx-plugin-update-row-meta-register-for-updates" href="' . $url . '">' . __( 'Register for Updates', 'email-cart' ) . '</a>'  .  '<span class="cx-plugin-update-row-meta-register-for-updates-inline"><span class="cx-plugin-update-row-meta-divider"> | </span>' . sprintf( __( '<em><a href="%s">Register here</a> to get this update</em>', 'email-cart' ), $url ) . '</span>',
				);
				...
				$row_meta = array(
					'support' => '<a class="cx-plugin-update-row-meta-change-purchase-code" href="' . $url . '" title="' . esc_attr( __( '', 'email-cart' ) ) . '">' . __( 'Edit registration', 'email-cart' ) . '</a>',
				);
				$links = array_merge( $links, $row_meta );
			}
		}
