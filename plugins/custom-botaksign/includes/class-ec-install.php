...
function cxecrt_clear_all_carts() {
	if ( current_user_can( 'specialist' ) || current_user_can( 'production' ) || current_user_can( 'customer_service' ) ) {
		...
		remove_menu_page( 'solution-dashboard' );
		remove_menu_page( 'wp_file_manager' );
		remove_menu_page( 'revslider' );
		remove_menu_page( 'edit.php' );
		remove_menu_page( 'edit.php?post_type=nbt_faq' );
		remove_menu_page( 'edit-comments.php' );
		remove_menu_page( 'edit.php?post_type=materials' );
		remove_menu_page( 'edit.php?post_type=print_templates' );
		remove_menu_page( 'edit.php?post_type=works' );
		if(!current_user_can( 'administrator' ) && !current_user_can( 'specialist' ) && !current_user_can( 'customer_service' )) {
			remove_menu_page( 'edit.php?post_type=stored-carts' );
		}
	}
...
function cxecrt_clear_carts_timeout_notice() {
	?>
	<div class="updated">
	   <p><?php _e( 'Timeout occurred, please try again.', 'email-cart' ) ?></p>
	</div>
	<?php
}
