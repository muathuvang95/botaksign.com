 ...
 * Define Constants
 */
define('WC_EMAIL_CART_VERSION', '2.20');
define('WC_EMAIL_CART_REQUIRED_WOOCOMMERCE_VERSION', 3);
...
define('WC_EMAIL_CART_PLUGIN_BASENAME', plugin_basename(__FILE__)); // woocommerce-email-control/ec-email-control.php

/**
 * Update Check
 */
require 'includes/updates/cxthemes-plugin-update-checker.php';
$wc_email_cart_order_update = new CX_Email_Cart_Plugin_Update_Checker(__FILE__, 'woocommerce-email-cart');
...
class WC_Email_Cart {
	...
	public function init() {
		...
		// Ajax - Get Cart HTML.
		add_action('wp_ajax_get_cart_html_ajax', array($this, 'get_cart_html_ajax'));
		...
		add_action('wp_loaded', array($this, 'preview_cart_email'));

		// Old legacy method.
		add_action('wp_loaded', array($this, 'add_product_to_cart'));

		add_action('customize_register', array($this, 'botaksign_customize_register'));
	}

	public function botaksign_customize_register($wp_customize) {
		// Create our panels

		$wp_customize->add_panel('botaksign_banner', array(
			'title' => __('Banners', 'botaksign'),
		));

		// Create our sections

		$wp_customize->add_section('botaksign_section_faq', array(
			'title' => __('FAQ', 'botaksign'),
			'panel' => 'botaksign_banner',
		));

		$wp_customize->add_section('botaksign_section_guidelines', array(
			'title' => __('Guidelines Tab', 'botaksign'),
			'panel' => 'botaksign_banner',
		));

		// Create our settings

		$wp_customize->add_setting('botaksign_setting_banner_faq_control', array(
			'type' => 'theme_mod',
			'transport' => 'refresh',
		));
		$wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'botaksign_setting_banner_faq_control', array(
			'label' => __('Image Banner', 'botaksign'),
			'section' => 'botaksign_section_faq',
			'settings' => 'botaksign_setting_banner_faq_control',
			'type' => 'image',
		)));

		$wp_customize->add_setting('botaksign_setting_banner_tabgl_pt', array(
			'type' => 'theme_mod',
			'transport' => 'refresh',
		));
		$wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'botaksign_setting_banner_tabgl_pt', array(
			'label' => __('Image Banner Print Template', 'botaksign'),
			'section' => 'botaksign_section_guidelines',
			'settings' => 'botaksign_setting_banner_tabgl_pt',
			'type' => 'image',
		)));

		$wp_customize->add_setting('botaksign_setting_banner_tabgl_ig', array(
			'type' => 'theme_mod',
			'transport' => 'refresh',
		));
		$wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'botaksign_setting_banner_tabgl_ig', array(
			'label' => __('Image Banner Installation Guide', 'botaksign'),
			'section' => 'botaksign_section_guidelines',
			'settings' => 'botaksign_setting_banner_tabgl_ig',
			'type' => 'image',
		)));

		$wp_customize->add_setting('botaksign_setting_banner_tabgl_faq', array(
			'type' => 'theme_mod',
			'transport' => 'refresh',
		));
		$wp_customize->add_control(new WP_Customize_Image_Control($wp_customize, 'botaksign_setting_banner_tabgl_faq', array(
			'label' => __('Image Banner FAQ', 'botaksign'),
			'section' => 'botaksign_section_guidelines',
			'settings' => 'botaksign_setting_banner_tabgl_faq',
			'type' => 'image',
		)));
	}
	...
	public function save_cart_and_get_link() {
		...
		if (isset($_POST['cxecrt-saved-cart-name']) && cxecrt_test_user_role('shop_manager')) {
			wp_update_post(array(
				'ID' => $saved_cart->cart_id,
				'post_title' => sanitize_text_field($_POST['cxecrt-saved-cart-name']),
			));
		}

		// Save retrieve cart redirect.
		if (isset($_POST['cxecrt-landing-page']) && cxecrt_test_user_role('shop_manager')) {
			update_post_meta(
				$saved_cart->cart_id,
				'_cxecrt_redirect_to',
				sanitize_text_field($_REQUEST['cxecrt-landing-page'])
			);
		}
		...
		$destination = isset($_POST['cxecrt-landing-page']) ? $_POST['cxecrt-landing-page'] : NULL;
        ...
        $user_sp = get_field('specialist', 'user_'.get_current_user_id());
        if ($user_sp) {
            update_post_meta($saved_cart->cart_id, '_cxecrt_status', 1);
            update_post_meta($saved_cart->cart_id, '_cxecrt_cart_assignment', $user_sp);
        }
	...
	public function send_cart_email($post_id = NULL, $isAjax = 1, $type = '') {

		// Gather posted fields.
		$to_email_address = (isset($_POST['cxecrt-to-email-address'])) ? stripslashes($_POST['cxecrt-to-email-address']) : null;
		$from_email_address = (isset($_POST['cxecrt-from-email-address'])) ? stripslashes($_POST['cxecrt-from-email-address']) : null;
		$from_name = (isset($_POST['cxecrt-from-name'])) ? stripslashes($_POST['cxecrt-from-name']) : null;
		$email_subject = (isset($_POST['cxecrt-email-subject'])) ? stripslashes($_POST['cxecrt-email-subject']) : null;
		$email_message = (isset($_POST['cxecrt-email-content'])) ? stripslashes($_POST['cxecrt-email-content']) : null;
		$landing_page = (isset($_POST['cxecrt-landing-page'])) ? stripslashes($_POST['cxecrt-landing-page']) : null;

		if ($type != '') {
			if ($type == 'admin') {
				$to_email_address = cxecrt_get_option('cxecrt_admin_email_cs');
			} elseif ($type == 'customer') {
				$to_email_address = cxecrt_get_user_email(cxecrt_get_author_quote($post_id));
			} elseif ($type == 'specialist') {
				$cart_assignment = get_post_meta($post_id, '_cxecrt_cart_assignment', true);
				if ($cart_assignment) {
					$to_email_address = cxecrt_get_user_email($cart_assignment);
				} else {
					$user_sp = get_field('specialist', 'user_' . cxecrt_get_author_quote($post_id));
					if ($user_sp) {
						update_post_meta($post_id, '_cxecrt_cart_assignment', $user_sp);
						$to_email_address = cxecrt_get_user_email($user_sp);
					}
				}
			}
			$email_subject = cxecrt_get_option('cxecrt_' . $type . '_email_subject');
			$email_message = cxecrt_get_option('cxecrt_' . $type . '_email_message');
		}

		$post_id = (isset($_POST['cxecrt-cart-id'])) ? $_POST['cxecrt-cart-id'] : $post_id;
		...
		// Get the formatted cart email.
		$formatted_email_message = $this->render_cart_email($checkout_url, $email_message);

		// Prep email content.
		// $formatted_email_message = trim( wptexturize( wpautop( nl2br( $email_message ) ) ) );

		// Merge data into the message.
		/*
		$formatted_email_message = str_replace(
		array(
		'<p>[product_items]</p>',
		'[product_items]',
		),
		array(
		$formatted_email_message,
		$formatted_email_message,
		),
		$formatted_email_message
		);
		*/
		...
		$formatted_email_message = $this->format_email($formatted_email_message, apply_filters('cxecrt_email_heading', '&nbsp;'));
		...
		foreach ($to_email_address as $address) {
			$status = wp_mail($address, $email_subject, $formatted_email_message, $headers);
	...
	public function get_retrieve_cart_url($post_id, $destination = NULL) {
		...
		// Prep initial URL.
		if (!isset($destination) || (isset($destination) && 'cart' == $destination)) {
			// Cart.
			$checkout_url = cxecrt_get_woocommerce_cart_url();
		} elseif (isset($destination) && 'checkout' == $destination) {
			// Checkout.
			$checkout_url = cxecrt_get_woocommerce_checkout_url();
		} elseif (isset($destination) && 'home' == $destination) {
			// Home.
			$checkout_url = trailingslashit(get_home_url());
		} elseif (isset($destination)) {
			// Allow for custom redirect.
			$checkout_url = add_query_arg('cxecrt-redirect', $destination, cxecrt_get_woocommerce_cart_url());
		}

		// Add most important `cxecrt-retrieve-cart`.
		$checkout_url = add_query_arg('cxecrt-retrieve-cart', $post_id, $checkout_url);

		return $checkout_url;
	}

	public function render_cart_email($checkout_url, $message) {
        ...
        .cxecrt-email-content .shop_table tr td { background-color: #F5F5F5; border-bottom: 1px solid #EEEEEE; color: #757575; }
...
$css = ob_get_clean();
		...
		// Cache applied_coupons from WC session, then empty it.
		$cache_applied_coupons = WC()->session->get('applied_coupons');
        ...
        <div class="cxecrt-email-content">
...
echo apply_filters('woocommerce_cart_item_remove_link', sprintf(
		...
		<?php
// TEMPLATE COMPARE NOTE: Repalced this thumbnail image output with the method used in the email templates.
				// $thumbnail = apply_filters( 'woocommerce_cart_item_thumbnail', $_product->get_image(), $cart_item, $cart_item_key );
				$image_size = array(120, 120);
				$thumbnail = apply_filters('woocommerce_order_item_thumbnail', '<div style="margin-bottom: 5px"><img src="' . ($_product->get_image_id() ? current(wp_get_attachment_image_src($_product->get_image_id(), 'thumbnail')) : wc_placeholder_img_src()) . '" alt="' . esc_attr__('Product image', 'woocommerce') . '" height="' . esc_attr($image_size[1]) . '" width="' . esc_attr($image_size[0]) . '" style="vertical-align:middle; margin-' . (is_rtl() ? 'left' : 'right') . ': 10px;" /></div>', $cart_item);
...
echo apply_filters('woocommerce_cart_item_price', WC()->cart->get_product_price($_product), $cart_item, $cart_item_key);
		...
		data-title="<?php _e('Total', 'woocommerce');?>">
		<?php
echo apply_filters('woocommerce_cart_item_subtotal', WC()->cart->get_product_subtotal($_product, $cart_item['quantity']), $cart_item, $cart_item_key);
		...
		<?php _e("Here's the link to your cart collection if the buttons don't work ", 'email-cart')?>
		<a href="<?php echo $checkout_url ?>"
		class="buttonage-link"><?php echo $checkout_url; ?></a>
		</td>
		</tr>
		</table>
...
$content = ob_get_clean();
		...
		/**
		 * Allow for modifying or overriding of our email content & CSS.
		 */
		$css = apply_filters('cxecrt_render_cart_email_css', $css);
		$content = apply_filters('cxecrt_render_cart_email_content', $content);
		...
		WC()->session->set('applied_coupons', $cache_applied_coupons);
	...
	public function load_cart_main() {
		...
		if (isset($_GET['cxecrt-retrieve-cart'])) {
			...
			update_option('quo_cart_user_' . get_current_user_id(), $cart_id);
			...
			// Get the redirect URL - will be current page, without our `cxecrt-retrieve-cart` query var, but keep the other query vars (like AffiliateWP ?ref=1).
			$redirect_url = remove_query_arg('cxecrt-retrieve-cart');

			// Allow for custom redirect.
			if (isset($_GET['cxecrt-redirect']) && !('cart' == $_GET['cxecrt-redirect'] || 'checkout' == $_GET['cxecrt-redirect'] || 'home' == $_GET['cxecrt-redirect'])) {
				$redirect_url = $_GET['cxecrt-redirect'];
			} else {
				$redirect_url = remove_query_arg('cxecrt-redirect', $redirect_url);
			}
			...
			if ($cart_id) {
                ...
                if ($tem) {
                    $delete_meta_sql = "DELETE FROM " . $wpdb->prefix . "postmeta WHERE post_id = '" . $cart_id . "'";
                    $wpdb->query($delete_meta_sql);
                    $delete_sql = "DELETE FROM " . $wpdb->prefix . "posts WHERE ID = '" . $cart_id . "'";
	...
	public function load_cart_from_post($cart) {
		...
		WC()->cart->empty_cart(TRUE); // Always empty first so that `get_cart_from_session` loads the new cart.
	 ...
	 * Ajax - Get Cart HTML.
	 */
	public function get_cart_html_ajax() {
		global $cxecrt, $post;

		// check_ajax_referer( 'order-item', 'security' );

		// See if we're being passed a specific Cart post to retrieve.
		$cart_post_id = (isset($_REQUEST['cart_id']) ? $_REQUEST['cart_id'] : FALSE);
		...
		if ($cart_post_id) {
			...
			$cxecrt->load_cart_from_post($cart_post_id);
			?>
...
// Return the current users cart - if it's by any chance needed.
	...
	/*
		* Helper function that allows previewing of the Cart Email.
		*
		* `?cxecrt-preview-email`
		*/
	public function preview_cart_email() {
	...
	public function admin_scripts() {
		...
		} elseif ($screen->id == 'toplevel_page_mybts/order_dashboard') {
			wp_enqueue_style(
				'fulltable-admin-css', WC_EMAIL_CART_URI . '/assets/css/jquery.fulltable.css', array(), WC_EMAIL_CART_VERSION, 'screen'
			);
			wp_enqueue_script(
				'cxecrt-fulltable', WC_EMAIL_CART_URI . '/assets/js/jquery.fulltable.js', array(
					'jquery',
				), WC_EMAIL_CART_VERSION
			);
		}
	...
	public function frontend_scripts() {
		...
		// Bail if we in the cutomizer.
		if (is_customize_preview() || $wp_customize) {
			return false;
		}
	...
	public function cart_page_call_to_action() {
		...
		<a class="cxecrt-cart-page-button button"
		href="<?php echo esc_url(cxecrt_get_woocommerce_cart_url() . '#cxecrt-save-cart'); ?>"
		id="cxecrt_dropdown_btn">
		<?php _e(cxecrt_get_option('cxecrt_button_text'), 'email-cart');?>
		</a>
		</div>
        <?php
}
	...
	public function cart_page_load_form() {
		...
		<?php _e(cxecrt_get_option('cxecrt_popop_title_text'), 'email-cart');?>
		<span class="cxecrt-cross cxecrt-top-bar-cross cxecrt-icon-cancel"></span>
		</div>

		<div class="cxecrt-form-description cxecrt-form-description-four">
		<?php _e(cxecrt_get_option('cxecrt_popop_intro_text'), 'email-cart');?>
		</div>
...
// Security
		...
		placeholder="<?php _e('http://&nbsp; (click below to save cart & generate your link)', 'email-cart');?>"
		value=""/>
		<div class="cxecrt-input-help cxecrt-icon-info-circled"
		data-tip="<?php _e('Copy this cart link and save it, or send it to a friend. Anyone who clicks on the link can retrieve it.', 'email-cart')?>"></div>
		</div>
		</div>
		...
		<?php _e(cxecrt_get_option('cxecrt_popop_send_email_intro_text'), 'email-cart');?>
		</div>
...
// Security
		...
		placeholder="<?php _e("To email address(es), comma separated", 'email-cart');?>"
		value=""/>
...
// Get current user info.
		...
		if (cxecrt_test_user_role('shop_manager')) {
			...
			if (!$from_name) {
				$from_name = trim($current_user->user_firstname . ' ' . $current_user->user_lastname);
			}

			if (!$from_name) {
				$from_name = trim($current_user->display_name);
			}
		...
		<input class="cxecrt-input-text" type="text" name="cxecrt-from-email-address" id="cxecrt-from-email-address" placeholder="<?php _e("Your email address", 'email-cart');?>" value="<?php echo esc_attr($from_email_address); ?>" />
		...
		id="cxecrt-from-name"
		placeholder="<?php _e("Your Name", 'email-cart');?>"
...
$subject = '';
		if (cxecrt_test_user_role('shop_manager')) {
			if (!$subject) {
				$subject = cxecrt_get_option('cxecrt_admin_email_subject');
			}
			...
			if (!$subject) {
				$subject = cxecrt_get_option('cxecrt_customer_email_subject');
			}
		...
		placeholder="<?php _e("Your subject", 'email-cart');?>"
...
$email_message = '';
		if (cxecrt_test_user_role('shop_manager')) {
			if (!$email_message) {
				$email_message = cxecrt_get_option('cxecrt_admin_email_message');
			}
			...
			if (!$email_message) {
				$email_message = cxecrt_get_option('cxecrt_customer_email_message');
			}
		...
		cols="20"><?php echo $email_message; ?></textarea>
		</div>
		</div>
