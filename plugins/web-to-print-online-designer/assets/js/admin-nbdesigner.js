...
jQuery(document).ready(function ($) {
    ...
    $('#_nbdesigner_enable').change(function() {
        ...
        $('#nbd_design_help_status').toggleClass('nbdesigner-disable'); //CS botak
        if($('#_nbdesigner_enable').prop("checked")){
            $('.nbdesigner-right.add_more').show();
        };        
    });
    $('#_designer_upload_cs').change(function() {
        $('#nbd_setting_options').toggleClass('nbdesigner-disable');
    });
...
var NBDESIGNADMIN = {
    ...
    loadImageOverlay: function(e){
        ...
        var config = this.initParameter(e); //CS botak overlay
        upload = wp.media.frames.file_frame = wp.media({
            title: 'Choose Image',
            button: {
                text: 'Choose Image'
        ...
        upload.on('select', function () {
            ...
            var overlay_real_width = attachment.width / convert_ratio,
                overlay_real_height = attachment.height / convert_ratio,
                overlay_real_top = config.vRealHeight - overlay_real_height > 0 ? config.vRealTop + (config.vRealHeight - overlay_real_height) / 2 : config.vRealTop,
                overlay_real_left = config.vRealWidth - overlay_real_width > 0 ? config.vRealLeft + (config.vRealWidth - overlay_real_width) / 2 : config.vRealLeft,
                overlay_width = overlay_real_width * config.ratio < config.vRelWidth ? overlay_real_width * config.ratio : config.vRelWidth,
                overlay_height = overlay_real_height * config.ratio < config.vRelWidth ? overlay_real_height * config.ratio : config.vRelHeight,
                overlay_top = overlay_real_top * config.ratio + config.proSize.top,
                overlay_left = overlay_real_left * config.ratio + config.proSize.left;
            jQuery(e).parents('.nbdesigner-box-collapse').find('.overlay_real_width').val(overlay_real_width);
            jQuery(e).parents('.nbdesigner-box-collapse').find('.overlay_real_height').val(overlay_real_height);
            jQuery(e).parents('.nbdesigner-box-collapse').find('.overlay_real_top').val(overlay_real_top);
            jQuery(e).parents('.nbdesigner-box-collapse').find('.overlay_real_left').val(overlay_real_left);
            jQuery(e).parents('.nbdesigner-box-collapse').find('.overlay_width').val(overlay_width);
            jQuery(e).parents('.nbdesigner-box-collapse').find('.overlay_height').val(overlay_height);
            jQuery(e).parents('.nbdesigner-box-collapse').find('.overlay_top').val(overlay_top);
            jQuery(e).parents('.nbdesigner-box-collapse').find('.overlay_left').val(overlay_left);
            jQuery(e).parents('.nbdesigner-box-collapse').find('.nbdesigner-image-overlay').css({'top': overlay_top + 'px', 'left': overlay_left + 'px', 'width': overlay_width + 'px',  'height': overlay_height + 'px'})
        });
        upload.open();
    },
    ...
    resetBoxes: function (command) {
        ...
        jQuery.each(jQuery(index + ' .nbdesigner-box-container'), function (key, val) {
            ...
            // CS-V3 Bleed and Safe
            jQuery(this).find('.bleed_top').attr('name', name + '[' + key + '][bleed_top]');
            jQuery(this).find('.bleed_left').attr('name', name + '[' + key + '][bleed_left]');
            jQuery(this).find('.bleed_bottom').attr('name', name + '[' + key + '][bleed_bottom]');
            jQuery(this).find('.bleed_right').attr('name', name + '[' + key + '][bleed_right]');
            jQuery(this).find('.bleed_corner_top').attr('name', name + '[' + key + '][bleed_corner_top]');
            jQuery(this).find('.bleed_corner_left').attr('name', name + '[' + key + '][bleed_corner_left]');
            jQuery(this).find('.bleed_corner_bottom').attr('name', name + '[' + key + '][bleed_corner_bottom]');
            jQuery(this).find('.bleed_corner_right').attr('name', name + '[' + key + '][bleed_corner_right]');
            ...
            jQuery(this).find('.show_safe_zone').attr('name', name + '[' + key + '][show_safe_zone]');
            jQuery(this).find('.safe_corner').attr('name', name + '[' + key + '][safe_corner]');
            // End 
    ...
    nbdesigner_move: function (e, command) {
        ...
        this.updateDesignAreaSize(e);//CS botak overlay size
        this.updateBleed(e);
...
//            });
    ...
    updateBleed: function(e){
        ...
        bleedEl.css({width: config.bleedSize.width + 'px',
                    ...
                    bottom: config.bleedSize.bottom + 'px',
                    right: config.bleedSize.right + 'px',
                });
        // if (jQuery('.bleed-corner').hasClass('active')) {
        //     bleedEl.css({
        //         'border-radius': 100 + '%'
        //     });
        // } else {
            bleedEl.css({
                'border-top-left-radius': config.bleedSize.border_radius_top + 'px',
                'border-top-right-radius': config.bleedSize.border_radius_left + 'px',
                'border-bottom-left-radius': config.bleedSize.border_radius_bottom + 'px',
                'border-bottom-right-radius': config.bleedSize.border_radius_right + 'px'
            });
        // }  
    },
    updateSafeZone: function(e){
        var zoneEl = jQuery(e).parents('.nbdesigner-box-container').find('.nbd-safe-zone'),
            ...
            zoneEl.css({width: config.safeZone.width + 'px',
                    ...
                    bottom: config.safeZone.bottom + 'px',
                    right: config.safeZone.right + 'px'
                });
        // if (jQuery('.safe_corner').hasClass('active')) {
        //     zoneEl.css({
        //     'border-radius': 100 + '%'
        //     });
        // }else {
            zoneEl.css({
            'border-top-left-radius': config.safeZone.border_safe_radius_top + 'px',
            'border-top-right-radius': config.safeZone.border_safe_radius_left + 'px',
            'border-bottom-left-radius': config.safeZone.border_safe_radius_bottom + 'px',
            'border-bottom-right-radius': config.safeZone.border_safe_radius_right + 'px'
            });
        // }
    },
    //CS botak overlay size
    updateOverlaySize: function(e){
        ...
        var overlay_real_width = config.overlay_real_width,
            overlay_real_height = config.overlay_real_height,
            overlay_real_top = config.vRealHeight - overlay_real_height > 0 ? config.vRealTop + (config.vRealHeight - overlay_real_height) / 2 : config.vRealTop,
            overlay_real_left = config.vRealWidth - overlay_real_width > 0 ? config.vRealLeft + (config.vRealWidth - overlay_real_width) / 2 : config.vRealLeft,
            overlay_width = overlay_real_width * config.ratio < config.vRelWidth ? overlay_real_width * config.ratio : config.vRelWidth,
            overlay_height = overlay_real_height * config.ratio < config.vRelWidth ? overlay_real_height * config.ratio : config.vRelHeight,
    ...
    toggleBleedCorner: function(e) {
        this.updateBleed(e);
        jQuery('.bleed-corner').toggleClass('active');
    },
    toggleSafeCorner: function(e) {
        this.updateSafeZone(e);
        jQuery('.safe_corner').toggleClass('active');
    },
...
//        });
    ...
    updateDesignAreaSize : function(e){
        var config = this.initParameter(e);
        parent = jQuery(e).parents('.nbdesigner-box-collapse');
        if( jQuery(e).attr('onclick') == 'NBDESIGNADMIN.updateDesignAreaSize(this)') {
            var width_save = parseFloat(parent.find('.real_width').val());
            var height_save = parseFloat(parent.find('.real_height').val());
            var left_save = parseFloat(parent.find('.real_left').val());
            var top_save = parseFloat(parent.find('.real_top').val());
            var vRealWidth = width_save,
            vRealHeight = height_save,
            vRealLeft = left_save,
            vRealTop = top_save;
        } else {
        ...
        var config = this.initParameter(e);
        this.updateOverlaySize(e);//CS botak overlay size
        this.updateBleed(e);
        this.updateSafeZone(e);
    },
    ...
    initParameter: function(e){
        var state_bleed, state_safe = false;
        let check_bleed = jQuery('input[type="checkbox"].bleed_corner'), check_safe = jQuery('input[type="checkbox"].safe_corner');
        if (check_bleed.prop("checked") == true) state_bleed = true;
        if (check_safe.prop("checked") == true) state_safe = true;
        ...
        var parent = jQuery(e).parents('.nbdesigner-box-collapse'),
            ...
            // CS-v3 Bleed and Safe
            // Bleed
            iBleedTop = parent.find('.bleed_top'),
            vBleedTop = parseFloat(iBleedTop.val()),  
            iBleedLeft = parent.find('.bleed_left'),
            vBleedLeft = parseFloat(iBleedLeft.val()),
            iBleedBottom = parent.find('.bleed_bottom'),
            vBleedBottom = parseFloat(iBleedBottom.val()),  
            iBleedRight = parent.find('.bleed_right'),
            vBleedRight = parseFloat(iBleedRight.val()),
            // Bleed Corner
            iBleedCornerTop = parent.find('.bleed_corner_top'),
            vBleedCornerTop = state_bleed ? 1000 : parseFloat(iBleedCornerTop.val()),  
            iBleedCornerLeft = parent.find('.bleed_corner_left'),
            vBleedCornerLeft = state_bleed ? 1000 : parseFloat(iBleedCornerLeft.val()),
            iBleedCornerBottom = parent.find('.bleed_corner_bottom'),
            vBleedCornerBottom = state_bleed ? 1000 : parseFloat(iBleedCornerBottom.val()),  
            iBleedCornerRight = parent.find('.bleed_corner_right'),
            vBleedCornerRight = state_bleed ? 1000 : parseFloat(iBleedCornerRight.val()),
            // Safe 
            iMarginTop = parent.find('.margin_top'),
            vMarginTop = parseFloat(iMarginTop.val()),
            iMarginLeft = parent.find('.margin_left'),
            vMarginLeft = parseFloat(iMarginLeft.val()),
            iMarginBottom = parent.find('.margin_bottom'),
            vMarginBottom = parseFloat(iMarginBottom.val()),
            iMarginRight = parent.find('.margin_right'),
            vMarginRight = parseFloat(iMarginRight.val()),
            // Safe Corner
            iSafeCornerTop = parent.find('.safe_corner_top'),
            vSafeCornerTop = state_safe ? 1000 : parseFloat(iSafeCornerTop.val()),  
            iSafeCornerLeft = parent.find('.safe_corner_left'),
            vSafeCornerLeft = state_safe ? 1000 : parseFloat(iSafeCornerLeft.val()),
            iSafeCornerBottom = parent.find('.safe_corner_bottom'),
            vSafeCornerBottom = state_safe ? 1000 : parseFloat(iSafeCornerBottom.val()),
            iSafeCornerRight = parent.find('.safe_corner_right'),
            vSafeCornerRight = state_safe ? 1000 : parseFloat(iSafeCornerRight.val()),
            // end
            design_area = parent.find('.nbdesigner-area-design'),
            overlay_area = parent.find('.nbdesigner-image-overlay'),
            overlay_real_width = parseFloat(parent.find('.overlay_real_width').val()), //CS botak overlay
            overlay_real_height = parseFloat(parent.find('.overlay_real_height').val()), //CS botak overlay
        ...
        var bleedSize = {
            width :  vRelWidth - 2 *  ((vBleedLeft + vBleedRight) / 2) * ratio,
            height :  vRelHeight - 2 *  ((vBleedTop + vBleedBottom) / 2)  * ratio,
            left: vRelLeft + vBleedLeft * ratio,
            top: vRelTop + vBleedTop * ratio,
            bottom: vRelTop + vBleedBottom * ratio,
            right: vRelLeft + vBleedRight * ratio,
            border_radius_top: vBleedCornerTop * ratio,
            border_radius_left: vBleedCornerLeft * ratio,
            border_radius_bottom: vBleedCornerBottom * ratio,
            border_radius_right: vBleedCornerRight * ratio
        };
        var safeZone = {
            width :  vRelWidth - 2 *  ((vBleedLeft + vBleedRight) / 2) * ratio  - 2 *  ((vMarginLeft + vMarginRight) / 2) * ratio,
            height :  vRelHeight - 2 *  ((vBleedTop + vBleedBottom) / 2) * ratio  - 2 *  ((vMarginTop + vMarginBottom) / 2) * ratio,
            left: vRelLeft + vBleedLeft * ratio + vMarginLeft * ratio,
            top: vRelTop + vBleedTop * ratio + vMarginTop * ratio,
            bottom: vRelTop + vBleedBottom * ratio + vMarginBottom * ratio,
            right: vRelTop + vBleedRight * ratio + vMarginRight * ratio,
            border_safe_radius_top: vSafeCornerTop * ratio,
            border_safe_radius_left: vSafeCornerLeft * ratio,
            border_safe_radius_bottom: vSafeCornerBottom * ratio,
            border_safe_radius_right: vSafeCornerRight * ratio
        };
        return {
            iProWidth : iProWidth,
            ...
            overlay_real_width : overlay_real_width, //CS botak overlay
            overlay_real_height : overlay_real_height, //CS botak overlay
            ratio : ratio,
            offset : offset,
