    ...
    /* Drag & Drop uplod file */
        ...
        awsUploadfile = function(fileName, file, folderName , formData , nbds_frontend , nbu_item_key , fileS3) {
            ...
            return bucket.upload(params, function(err, data) {
                ...
                fileS3.tmp_name = data.Location;
                formData.delete('action');
                formData.append('action', 'nbd_upload_design_file');
                formData.append('nbu_item_key', nbu_item_key);
                formData.append('links3', data.Location);
                formData.append('file_name', fileName);
                formData.append('file_s3', JSON.stringify(fileS3));
                jQuery.ajax({
                    ...
                    success: function(res) {
                        if( res.flag == 1 ){
                            ...
                            listFileUpload.push( { src : res.src, name : res.name } );
                            buildPreviewUpload();
                        }else{
                            bucket.deleteObject(params_remove, function(err, data) {
                              if (err) console.log(err, err.stack);  // error
                              else      alert(res.mes);                // deleted
                            });
                        }
        ...
        function uploadFile(files){
            ...
            formData.append('action', 'nbd_check_before_upload_design_file');
            formData.append('task', 'new');
            formData.append('product_id', product_id);
            formData.append('variation_id', variation_id);
            formData.append('nonce', nbds_frontend.nonce);
            formData.append('file_name_s3', file.name);
            // S3
            var fileS3 = {};
            fileS3.size = file.size;
            fileS3.name = file.name;
            formData.append('file_name', file.name);
            var fullPath = file;
            var progressDiv = jQuery('.nbd-progress-bar');
            progressDiv.addClass('active');
            var progressBar = jQuery('.nbd-progress-bar-inner');
            var progressIndicator = jQuery('.nbd-progress-bar-inner .indicator');
            progressIndicator.css('left' , 'calc(0% - 15px)' );
            progressIndicator.html('0');
            jQuery.ajax({
                url: nbds_frontend.url,
                method: "POST",
                dataType: 'json',
                ...
                success: function(data) {
                    if( data.flag == 1 ){
                        folderName = 'reupload-design/'+ data.nbu_item_key+'/';
                        var fileName = data.new_name;
                        awsUploadfile(fileName, file, folderName  , formData , nbds_frontend , data.nbu_item_key , fileS3).on('httpUploadProgress', function(progress) {
                            let progressPercentage = Math.round(progress.loaded / progress.total * 100);
                            progressBar.css('width' , progressPercentage+'%' );
                            progressIndicator.css('left' , 'calc(' + progressPercentage + '% - 15px)' );
                            progressIndicator.html( progressPercentage );
...
var NBDESIGNERPRODUCT = {
    ...
    show_design_thumbnail: function (arr, task, config) {
        ...
        jQuery.each(arr, function (key, val) {
            ...
            var data_width = has_config ? product[p_index].img_src_width : '500',
                ...
                //CS botak add name for design side
            html += '<div class="img-con" style="cursor: pointer;" onclick="NBDESIGNERPRODUCT.show_lightbox( this )"><img data-title="'+data_title+'" data-width="'+data_width+'" data-height="'+data_height+'" src="' + val + '?t=' + d.getTime() +'" /><p class="design-side-name">' + data_title + '</p></div>'
        });
        jQuery.each( jQuery('#nbd-share-group a'), function(){
            var d = new Date();
            var href = jQuery(this).attr('data-href');
    ...
    show_design_thumbnail_without_hide_design_frame: function (arr, task, config) {
        if( jQuery('#triggerDesign').length > 0 ){
            jQuery('button[type="submit"].single_add_to_cart_button').show();
        };
        jQuery('#nbdesigner-preview-title').show();
