...
if(NBDESIGNCONFIG['ui_mode'] != 3){
    nbdApp = angular.module('nbd-app', ["angularSpectrumColorpicker"]);
}else{
    nbdApp = angular.module('nbdApp', ["angularSpectrumColorpicker"]);
};
...
nbdApp.controller('designCtrl', ['$scope', 'FabricWindow', '$window', 'NBDDataFactory', 'filterFontFilter', 'filterArtFilter', '$timeout', '$http', '$document', '$interval',
    ...
    $scope.defaultConfig = {
        ...
        showOverlay: 0,
        showSafeZone: 0,
        // CS-V3 Bleed line and Safe corner radius
        bleedCornerRadius: 0,
        safeCornerRadius: 0
    };
...
//            }
    ...
    $scope.onClickTab = function(type, tab){
        ...
        switch(type){
            ...
            case 'login-gallery':
                ...
                $scope.resource.photo.type = 'login-gallery';
                break;
            default:
                $scope.resource.element.type = type;
                break;
    ...
    $scope.onClickTabLoginGallery = function() {
        jQuery('#nb-tab-gallery').triggerHandler('click');
        $scope.resource.photo.onclick = !$scope.resource.photo.onclick;
        $scope.toggleStageLoading();
        NBDDataFactory.get('nbd_get_user_id', {type: 'get_user_id'}, function(data) {
            data = JSON.parse(data);
            NBDESIGNCONFIG['user_id'] = data.user_id;
            $scope.toggleStageLoading();
        })
    }
        ...
        switch( type ){
            case 'flaticon':
                function getFlaticon(){
                    $http({
                        method: "GET",
                        url: 'https://api.flaticon.com/v2/search/icons/selection?q=' + search + '&page=' + $scope.resource.flaticon.filter.currentPage + '&limit=' + $scope.resource.flaticon.filter.perPage,
                        headers: {
        ...
        if( $scope.resource.photo.filter.totalPage == 0 || $scope.resource.photo.filter.currentPage <= $scope.resource.photo.filter.totalPage ){
            switch(type){
                ...
                case 'Pexels':
                    ...
                    break;
                case 'library': //CS download design
                    var url = $scope.resource.photo.photoSearch != '' ? NBDESIGNCONFIG['home_url'] + '/wp-json/nbd/v1/library?user_id=' + NBDESIGNCONFIG['user_id'] + '&query=' + encodeURIComponent( $scope.resource.photo.photoSearch ) + '&per_page='+$scope.resource.photo.filter.perPage + '&page=' + $scope.resource.photo.filter.currentPage : NBDESIGNCONFIG['home_url'] + '/wp-json/nbd/v1/library?user_id=' + NBDESIGNCONFIG['user_id'] + '&per_page=' + $scope.resource.photo.filter.perPage + '&page='+$scope.resource.photo.filter.currentPage;
                    $http({
                        method: 'GET',
                        url: url
                    }).then(function successCallback(response) {
                        var data = response.data,
                        totalPage = data.pagesTotal;
                        $scope.resource.photo.filter.totalPage = totalPage > 10 ? 10 : totalPage;
                        _.each(data.gallery, function(val, key) {
                            $scope.resource.photo.data.push({
                                extenal: 1,
                                img_id: val.id,
                                ...
                                preview: val.src,
                                type: 'image-gallery',
                                is_free: typeof val.price != "undefined" ? (0 == parseFloat(val.price) ? true : false) : true,
                                img_price: typeof val.price != "undefined" ? parseFloat(val.price) : 0,
                                is_paid: val.is_paid
                            });
                        });
                        $scope.afterGetResource('photo');
                    }, function errorCallback(response) {
                        alert('Error when load library image!');
                        jQuery('#tab-photo .loading-photo').hide();
                    }); 
        ...
        switch(type){
            case 'clipart':
                ...
                if( angular.isDefined( $scope.settings.nbes_enable_settings ) && $scope.settings.nbes_enable_settings.clipart == 1 
                    ...
                    $scope.resource.clipart.data.cat = $scope.resource.clipart.data.cat.filter(function(cat){
                        var cid = cat.id;
                        return $scope.settings.nbes_settings.clipart_cats.indexOf(cid) > -1;
                    });
                }
...
//            }
            ...
            if( angular.isDefined( data.size ) ){
                angular.forEach($scope.settings.product_data.product, function(side, key){
                    ...
                    // CS-V3 BLeed line and Safe corner radius
                    ['safe_corner_radius','safe_corner_margin_top' , 'safe_corner_margin_bottom' , 'safe_corner_margin_left' ,'safe_corner_margin_right','bleed_top','bleed_bottom'
                    , 'bleed_left', 'bleed_right' , 'margin_top', 'margin_bottom' ,'margin_left' , 'margin_right'
                    , 'show_bleed', 'bleed_corner_radius' , 'bleed_corner_left' , 'bleed_corner_top' , 
                    'bleed_corner_right' , 'bleed_corner_bottom',  
                    'bleed_radius_left' , 'bleed_radius_top' , 'bleed_radius_right' , 'bleed_radius_bottom',

                    'safe_radius_left' , 'safe_radius_top' , 'safe_radius_right' , 'safe_radius_bottom',

                    ,'show_safe_zone', 'show_overlay' , 'img_overlay'].map(function (att) { //CS botak option
                    ...
                    // CS-V3 Bleed and Safe corner.
                    if( data.size.show_bleed ){

                        // Bleed
                        side.bleed_radius_top = parseFloat( data.size.bleed_corner_top );
                        side.bleed_radius_bottom = parseFloat( data.size.bleed_corner_bottom );
                        side.bleed_radius_left = parseFloat( data.size.bleed_corner_left );
                        side.bleed_radius_right = parseFloat( data.size.bleed_corner_right );
                        // Safe 
                        side.safe_radius_top = parseFloat( data.size.safe_corner_margin_top );
                        side.safe_radius_bottom = parseFloat( data.size.safe_corner_margin_bottom );
                        side.safe_radius_left = parseFloat( data.size.safe_corner_margin_left );
                        side.safe_radius_right = parseFloat( data.size.safe_corner_margin_right );

                        if (data.size.safe_corner_radius == 1) {
                            side.safezone_radius_top_px = 1000;
                            side.safezone_radius_left_px = 1000;
                            side.safezone_radius_right_px = 1000;
                            side.safezone_radius_bottom_px = 1000;
                        }
                        else {
                            // Safe 
                            if( side.safe_radius_top != 0 ){
                                side.safezone_radius_top_px = $scope.stages[key].config.cwidth * side.safe_radius_top / side.product_width;
                            }else if (side.safe_radius_top == 0) {
                                side.safezone_radius_top_px = 0;
                            }
                            if( side.safe_radius_bottom != 0 ){
                                side.safezone_radius_bottom_px = $scope.stages[key].config.cwidth * side.safe_radius_bottom / side.product_width;
                            }else if (side.bleed_radius_bottom == 0) {
                                side.safezone_radius_bottom_px = 0;
                            }
                            if( side.safe_radius_left != 0 ){
                                side.safezone_radius_left_px = $scope.stages[key].config.cwidth * side.safe_radius_left / side.product_width;
                            }else if (side.bleed_radius_left == 0) {
                                side.safezone_radius_left_px = 0;
                            }
                            if( side.safe_radius_right != 0 ){
                                side.safezone_radius_right_px = $scope.stages[key].config.cwidth * side.safe_radius_right / side.product_width;
                            }else if (side.bleed_radius_right == 0) {
                                side.safezone_radius_right_px = 0;
                            }
                        }


                        if (data.size.bleed_corner_radius == 1) {
                            side.bleed_radius_top_px = 1000;
                            side.bleed_radius_bottom_px = 1000;
                            side.bleed_radius_left_px = 1000;
                            side.bleed_radius_right_px = 1000;
                        } else {
                            if( side.bleed_radius_top != 0 ){
                                side.bleed_radius_top_px = $scope.stages[key].config.cwidth * side.bleed_radius_top / side.product_width;
                            }else if (side.bleed_radius_top == 0) {
                                side.bleed_radius_top_px = 0;
                            }
                            if( side.bleed_radius_bottom != 0 ){
                                side.bleed_radius_bottom_px = $scope.stages[key].config.cwidth * side.bleed_radius_bottom / side.product_width;
                            }else if (side.bleed_radius_bottom == 0) {
                                side.bleed_radius_bottom_px = 0;
                            }
                            if( side.bleed_radius_left != 0 ){
                                side.bleed_radius_left_px = $scope.stages[key].config.cwidth * side.bleed_radius_left / side.product_width;
                            }else if (side.bleed_radius_left == 0) {
                                side.bleed_radius_left_px = 0;
                            }
                            if( side.bleed_radius_right != 0 ){
                                side.bleed_radius_right_px = $scope.stages[key].config.cwidth * side.bleed_radius_right / side.product_width;
                            }else if (side.bleed_radius_right == 0) {
                                side.bleed_radius_right_px = 0;
                            }
                        }          
                    }
                });
            ...
            if( angular.isDefined( data.orientation ) ){
                angular.forEach($scope.settings.product_data.product, function(side, key){
                    ...
                    if( ( data.orientation == 1 && productWidth < productHeight ) || ( data.orientation == 0 && productHeight < productWidth ) ){
                        ...
                        // CS-V3 add new bleed 
                        tem1 = 1 * side.bleed_top;
                        side.bleed_top = 1 * side.bleed_left;
                        side.bleed_left = tem1;

                        tem2 = 1 * side.bleed_bottom;
                        side.bleed_bottom = 1 * side.bleed_right;
                        side.bleed_right = tem2;
                        // CS-V3 Bleed line and Safe corner radius
                        tem1 = 1 * side.bleed_corner_top;
                        side.bleed_corner_top = 1 * side.bleed_corner_left;
                        side.bleed_corner_left = tem1;
                        tem2 = 1 * side.bleed_corner_bottom;
                        side.bleed_corner_bottom = 1 * side.bleed_corner_right;
                        side.bleed_corner_right = tem2;
                        tem1 = 1 * side.safe_corner_margin_top;
                        side.safe_corner_margin_top = 1 * side.safe_corner_margin_left;
                        side.safe_corner_margin_left = tem1;
                        tem2 = 1 * side.safe_corner_margin_bottom;
                        side.safe_corner_margin_bottom = 1 * side.safe_corner_margin_right;
                        side.safe_corner_margin_right = tem2;
        ...
        if( angular.isDefined( nbd_window.nbOption ) && angular.isDefined( nbd_window.nbOption.extraOdOption ) ){
            ...
            if( angular.isDefined( data.rounded_corner ) ){
                ...
                angular.forEach($scope.settings.product_data.product, function(side, key){
                    ...
                    side.safezone_radius = parseFloat( data.rounded_corner ) - ((side.margin_left + side.margin_right) / 2 );
                    ...
                    }else if (side.bleed_radius == 0) {
                        $scope.stages[key].config.bleed_radius = 25;
                        $scope.stages[key].config.safezone_radius = 25;
                    }
                });
            } 
    ...
    $scope.saveData = function(type){
        ...
        switch(type){
            ...
            default:
                ...
                ['product_id', 'variation_id', 'task', 'task2', 'design_type', 'nbd_item_key', 'cart_item_key', 'order_id', 'enable_upload_without_design', 'auto_add_to_cart', 'ui_mode' , 'item_id' , 'reupload'].forEach(function(key){
...
//                    dataObj.qtys = $scope.resource.config.qtys;
                ...
                $timeout(function(){
                    ...
                    dataObj['images_paid'] = [];
                    $scope.dataImage.images_paid.map(image => {
                        dataObj['images_paid'].push(image.img_id);
                    })
                    ...
                    NBDDataFactory.get(action, dataObj, function(data){
                        ...
                        if(data.flag == 'success'){
                            ...
                            nbd_window.jQuery('button[type="submit"].single_add_to_cart_button').show();
                            ...
                            $timeout(function(){
                                // nbd_window.jQuery('.single_add_to_cart_button').show();
                                nbd_window.jQuery('button[type="submit"].single_add_to_cart_button').show();
                            });
                            ...
                            // BOTAK custom reupload phase 3
                            if(NBDESIGNCONFIG['reupload'] == 'reupload'){
                                window.location = NBDESIGNCONFIG['redirect_url']+'&save_design_reorder=1&nbd_item_key='+data.folder;
                                return;
                            };
    ...
    $scope.saveDesign = function( downloadType ){
        ...
        _.each($scope.stages, function(stage, index){
            ...
            if( NBDESIGNCONFIG.enable_sticker_preview && index == 0 ){
...
//        }
    ...
    $scope.processProductSettings = function(){
        ...
        _.each($scope.settings.product_data.product, function(side, index){
            ...
            if ( !angular.isDefined( nbd_window.nbOption) || !angular.isDefined( nbd_window.nbOption.odOption) || !angular.isDefined( nbd_window.nbOption.odOption.size) || !nbd_window.nbOption.odOption.size) {
                if (angular.isDefined( side.bleed_corner) && side.bleed_corner == 1) {
                    side.bleed_radius_top_px = 1000;
                    side.bleed_radius_bottom_px = 1000;
                    side.bleed_radius_left_px = 1000;
                    side.bleed_radius_right_px = 1000;
                } else {
                    if( side.bleed_corner_top != 0 ){
                        side.bleed_radius_top_px = designViewPort.width * side.bleed_corner_top / side.product_width;
                    }else if (side.bleed_corner_top == 0) {
                        side.bleed_radius_top_px = 0;
                    }
                    if( side.bleed_corner_bottom != 0 ){
                        side.bleed_radius_bottom_px = designViewPort.width * side.bleed_corner_bottom / side.product_width;
                    }else if (side.bleed_corner_bottom == 0) {
                        side.bleed_radius_bottom_px = 0;
                    }
                    if( side.bleed_corner_left != 0 ){
                        side.bleed_radius_left_px = designViewPort.width * side.bleed_corner_left / side.product_width;
                    }else if (side.bleed_corner_left == 0) {
                        side.bleed_radius_left_px = 0;
                    }
                    if( side.bleed_corner_right != 0 ){
                        side.bleed_radius_right_px = designViewPort.width * side.bleed_corner_right / side.product_width;
                    }else if (side.bleed_corner_right == 0) {
                        side.bleed_radius_right_px = 0;
                    }
                }

                if (angular.isDefined( side.safe_corner) && side.safe_corner == 1) {
                    side.safezone_radius_top_px = 1000;
                    side.safezone_radius_left_px = 1000;
                    side.safezone_radius_right_px = 1000;
                    side.safezone_radius_bottom_px = 1000;
                    ...
                    // Safe 
                    if( side.safe_corner_top != 0 ){
                        side.safezone_radius_top_px = designViewPort.width * side.safe_corner_top / side.product_width;
                    }else if (side.safe_corner_top == 0) {
                        side.safezone_radius_top_px = 0;
                    }
                    if( side.safe_corner_bottom != 0 ){
                        side.safezone_radius_bottom_px = designViewPort.width * side.safe_corner_bottom / side.product_width;
                    }else if (side.safe_corner_bottom == 0) {
                        side.safezone_radius_bottom_px = 0;
                    }
                    if( side.safe_corner_left != 0 ){
                        side.safezone_radius_left_px = designViewPort.width * side.safe_corner_left / side.product_width;
                    }else if (side.safe_corner_left == 0) {
                        side.safezone_radius_left_px = 0;
                    }
                    if( side.safe_corner_right != 0 ){
                        side.safezone_radius_right_px = designViewPort.width * side.safe_corner_right / side.product_width;
                    }else if (side.safe_corner_right == 0) {
                        side.safezone_radius_right_px = 0;
                    }
                }
            }
            ...
            $scope.stages[index] = {
                config: {
                    ...
                    bleed_l: designViewPort.width * side.bleed_left / side.product_width,
                    bleed_r: designViewPort.width * side.bleed_right / side.product_width,

                    bleed_t: designViewPort.width * side.bleed_top / side.product_width,
                    bleed_b: designViewPort.width * side.bleed_bottom / side.product_width,



                    margin_l: designViewPort.width * side.margin_left / side.product_width,
                    margin_t: designViewPort.width * side.margin_top / side.product_width,

                    margin_r: designViewPort.width * side.margin_right / side.product_width,
                    margin_b: designViewPort.width * side.margin_bottom / side.product_width,

                    // CS-V3 Bleed line and Safe corner radius
                    bleed_radius_top: side.bleed_radius_top_px,
                    bleed_radius_left: side.bleed_radius_left_px,

                    bleed_radius_bottom: side.bleed_radius_bottom_px,
                    bleed_radius_right: side.bleed_radius_right_px,

                    
                    safe_radius_top: side.safezone_radius_top_px,
                    safe_radius_left: side.safezone_radius_left_px,

                    safe_radius_bottom: side.safezone_radius_bottom_px,
                    safe_radius_right: side.safezone_radius_right_px,



                    bleed_corner_l: designViewPort.width * side.bleed_corner_left / side.product_width,
                    bleed_corner_t: designViewPort.width * side.bleed_corner_top / side.product_width,

                    bleed_corner_r: designViewPort.width * side.bleed_corner_right / side.product_width,
                    bleed_corner_b: designViewPort.width * side.bleed_corner_bottom / side.product_width,

                    margin_corner_l: designViewPort.width * side.safe_corner_margin_left / side.product_width,
                    margin_corner_t: designViewPort.width * side.safe_corner_margin_top / side.product_width,

                    margin_corner_r: designViewPort.width * side.safe_corner_margin_right / side.product_width,
                    margin_corner_b: designViewPort.width * side.safe_corner_margin_bottom / side.product_width,
                    ...
                    // CS-V3 Bleed line and Safe corner radius
                    bleedCornerRadius: side.bleed_corner_radius,

                    showOverlay: side.show_overlay,
                    overlay_height: designViewPort.width *  side.overlay_real_height / side.product_width,
                    overlay_width: designViewPort.width *  side.overlay_real_width / side.product_width,
                    overlay_top: designViewPort.width *  side.overlay_real_top / side.product_width,
                    overlay_left: designViewPort.width *  side.overlay_real_left / side.product_width,
                    showSafeZone: side.show_safe_zone,
                    // CS-V3 Bleed line and Safe corner radius
                    safeCornerRadius: side.safe_corner_radius,
    ...
    /**
     * [changeAreaDesignShape description]
     */
    ...
    $scope.processAreaDesignShape = function(index){
        ...
        $timeout(function(){
            ...
            var _bleed_lr = (stage.config.bleed_l + stage.config.bleed_r) / 2;
            var _bleed_tb = (stage.config.bleed_t + stage.config.bleed_b) / 2;
            var _margin_lr = (stage.config.margin_l + stage.config.margin_r) / 2;
            var _margin_tb = (stage.config.margin_t + stage.config.margin_b) / 2;
            var bleedWidth = stage.states.scaleRange[stage.states.currentScaleIndex].ratio * (stage.config.width - 2 * _bleed_lr),
            bleedHeight = stage.states.scaleRange[stage.states.currentScaleIndex].ratio * (stage.config.height - 2 * _bleed_tb),
            safeZoneWidth = stage.states.scaleRange[stage.states.currentScaleIndex].ratio * (stage.config.width - 2 * _bleed_lr - 2 * _margin_lr),
            safeZoneHeight = stage.states.scaleRange[stage.states.currentScaleIndex].ratio * (stage.config.height - 2 * _bleed_tb - 2 * _margin_tb),
...
//        });
    ...
    $scope.onObjectAdded = function(id, options){
        ...
        item.set({
            ...
            'img_title': item.get('img_title'),
            'is_free': item.get('is_free'),
            'img_price': item.get('img_price'),
    ...
    $scope.showDesignTab = function(){
        if( checkMobileDevice() ){
            ...
            jQuery('.tabs-content').addClass('nbd-prevent-click');
        };    
    };
...
//        }
    ...
    $scope.onSelectionChanged = function(id, options){
        ...
        var item = options.target;//console.log(item.getSelectionStyles());
        $scope.updateUploadZone(item);
        $scope.updateWarning(item);
...
//                                }
                                var _src = item.src;
                                var url_s3 = '';
                                if(_src.indexOf('amazonaws.com') > 0) {
                                    url_s3 = _src;
                                } else {
                                    ...
                                    // fix image s3
                                    url_s3 = 'https://botaksignorder.s3.ap-southeast-1.amazonaws.com' + src[1];
                                }
                                item.src = url_s3;
                            };
...
//            });
            ...
            function _clone(__clipboard, mask){
                $timeout(function(){
                    __clipboard.clone(function(clonedObj) {
                        if( angular.isDefined(activeObject.get('origin_url')) && angular.isDefined(activeObject.get('origin_width')) && angular.isDefined(activeObject.get('origin_height')) ) {
                            clonedObj.set({
                                origin_url: activeObject.get('origin_url'),
                                origin_width: activeObject.get('origin_width'),
                                origin_height: activeObject.get('origin_height'),
                            });
                        }
                        ...
                        if (clonedObj.type === 'activeSelection') {
                            ...
                            clonedObj.forEachObject(function(obj, index) {
                                ...
                                if(angular.isDefined(objects[index])) {
                                    var objectByIndex = objects[index];
                                    if( angular.isDefined(objectByIndex.get('origin_url')) && angular.isDefined(objectByIndex.get('origin_width')) && angular.isDefined(objectByIndex.get('origin_height')) ) {
                                        obj.set({
                                            origin_url: objectByIndex.get('origin_url'),
                                            origin_width: objectByIndex.get('origin_width'),
                                            origin_height: objectByIndex.get('origin_height'),
                                        });
                                    }
                                 }
    ...
    $scope.updateLayersList = function(){
        ...
        _canvas.forEachObject(function(obj, index) {
            ...
            if ('undefined' !== typeof obj.get('img_id')) { //CS download design
                layerInfo['img_id'] = obj.get('img_id');
                layerInfo['img_title'] = obj.get('img_title');
                layerInfo['is_free'] = obj.get('is_free');
                layerInfo['img_price'] = obj.get('img_price');
                layerInfo['is_paid'] = obj.get('is_paid');
                //Get list image has price
                if (!layerInfo['is_free'] && !layerInfo['is_paid']) {
                    var added = false;
                    $scope.dataImage.images_unpaid.map(img => {
                        if (img.img_id === obj.img_id) {
                            img.quantity++;
                            added = true;
                        }
                    })
                    if (!added) {
                        $scope.dataImage.images_unpaid.push(
                            {
                                img_id: obj.get('img_id'),
                                img_title: obj.get('img_title'),
                                img_url: obj.get('src'),
                                img_price: obj.get('img_price'),
                                quantity: 1
                            }
                        )
                        $scope.dataImage.total += obj.get('img_price');
                    }
                    
//                    NBDDataFactory.get('gallery_get_image_info', {img_id: obj.get('img_id')}, function(data){
//                        data = JSON.parse(data);
//                        if (!data.is_bought) {
//                            //Save list img be used with price
//                            var added = false;
//                            $scope.dataImage.map(img => {
//                                if (img.img_id === data.img_id) {
//                                    img.quantity++;
//                                    added = true;
//                                }
//                            })
//                            if (!added) {
//                                $scope.dataImage.push(
//                                    {
//                                        img_id: data.img_id,
//                                        img_url: data.img_url,
//                                        img_price: data.img_price,
//                                        quantity: 1
//                                    }
//                                )
//                            }
//                        }
//                    })
                } else if (layerInfo['is_paid']) {
                    var added = false;
                    $scope.dataImage.images_paid.map(img => {
                        if (img.img_id === obj.img_id) {
                            img.quantity++;
                            added = true;
                        }
                    ...
                    if (!added) {
                        $scope.dataImage.images_paid.push(
                            {
                                img_id: obj.get('img_id'),
                                img_title: obj.get('img_title'),
                                img_url: obj.get('src'),
    ...
    $scope.initEyeDropper2 = function($event, global){
        ...
        jQuery('.nbd-eyedropper-inner-wrap').on('mouseenter', function(){
            jQuery(this).addClass('active');
        });
        jQuery('.nbd-eyedropper-inner-wrap').on('mouseleave', function(){
        ...
        jQuery('.nbd-eyedropper-wrap').on('mousedown', function(evt){
            jQuery(this).removeClass('active');
        });
    };
...
//            width: $scope.stages[$scope.currentStage]['canvas'].width * 0.9,
    ...
    /*CS download design, check is image has fee, add price to payment*/
    $scope.checkAndAddImage = function(img) {
//        if (img.type == 'image-gallery') {
//            console.log(img);
//        }
        $scope.resource.addImageContext = 'manual';
        $scope.addImageFromUrl(img)
    }
    /* Extenal Image */
    ...
    $scope.addImage = function(data, showLoading, hideLoading, additionalTitle, additionalValue, maskId ){
        var stage = $scope.stages[$scope.currentStage],
            _canvas = stage['canvas'],
            scale = stage.states.scaleRange[stage.states.currentScaleIndex].ratio,
            url = typeof data == 'object' ? ( angular.isDefined( data.src ) ? data.src : data.url ) : data; //(CS botak) fix bug add image
        ...
        fabric.Image.fromURL(url, function(op) {
            if( angular.isDefined( maskId ) || $scope.stages[$scope.currentStage].states.elementUpload ){
                ...
                //CS download design
                if( typeof data == 'object' && typeof 'undefined' != data.img_id ) {
                    op.set({
                        img_id: data.img_id,
    ...
    $scope.updateMapUrl = function(){
        if( $scope.resource.maps.address != '' ){
            ...
            $scope.resource.maps.url += address +'&zoom='+ $scope.resource.maps.zoom +'&scale=1&size='+ width +'x'+ height;
    ...
    $scope.saveDataAndDownload = function(type) {
        jQuery('.variations_form, form.cart').find('[name="nbo-ignore-design"]').remove();
...
//                    dataObj.qtys = $scope.resource.config.qtys;
                ...
                $timeout(function(){
                    //var action = ( type != 'preview_mockup' && type != 'share' && type != 'saveforlater' && type != 'download-pdf' && NBDESIGNCONFIG.task == 'new' && NBDESIGNCONFIG.ui_mode == 2 ) ?  'nbd_save_cart_design' : 'nbd_save_customer_design';
                    var action = ( !_.includes(excludeType, type) && NBDESIGNCONFIG.task == 'new' && NBDESIGNCONFIG.ui_mode == 2 ) ?  'nbd_save_cart_design' : 'nbd_save_customer_design';
                    action = 'nbd_save_and_download_customer_design';
                    if(NBDESIGNCONFIG.show_nbo_option == "1" && NBDESIGNCONFIG.task == 'new'){
                        action = 'nbd_save_and_download_customer_design';
                    }
                    if(type == 'save_draft'){
                    ...
                    if (NBDESIGNCONFIG['download_type'] != '' && typeof NBDESIGNCONFIG['download_type'] != 'undefined') {
                        dataObj['download-type'] = NBDESIGNCONFIG['download_type'];
                    }
                    NBDDataFactory.get(action, dataObj, function(data){
                        data = JSON.parse(data);
                        if(data.flag == 'success'){
                            ...
                            if( type == 'share' ) {
                                ...
                                if( NBDESIGNCONFIG['nbdesigner_auto_add_cart_in_detail_page'] == "yes" &&  NBDESIGNCONFIG.task == 'new' && NBDESIGNCONFIG.ui_mode == 1 && !( angular.isDefined( NBDESIGNCONFIG.edit_option_mode ) && NBDESIGNCONFIG.edit_option_mode == '1' ) ){
                                    ...
                                    nbd_window.NBDESIGNERPRODUCT.variation_id = NBDESIGNCONFIG['variation_id'];
                                    nbd_window.NBDESIGNERPRODUCT.folder = data.folder;
                                    nbd_window.NBDESIGNERPRODUCT.show_design_thumbnail_without_hide_design_frame(data.image, NBDESIGNCONFIG['task'], $scope.resource.config);
                                    nbd_window.NBDESIGNERPRODUCT.get_sugget_design(NBDESIGNCONFIG['product_id'], NBDESIGNCONFIG['variation_id']);   
                                    if( NBDESIGNCONFIG.ui_mode == 1 && $scope.resource.config.qty != null ){
                                        //nbd_window.NBDESIGNERPRODUCT.updateQty($scope.resource.config.qty);
                                    }
                            ...
                            //save link download file as base64 encode
                            $scope.dataDownload.downloadImageUrl = '';
                            if (data.dlf != '' && typeof data.dlf  != 'undefined') {
                                $scope.dataDownload.downloadImageUrl = data.dlf;
                                document.getElementById("nb-btn-submit-payment-stripe").removeAttribute("disabled");
                                document.getElementById("nb-btn-submit-payment-paypal").removeAttribute("disabled");
                            }
                        }else{
    ...
    $scope.submitStripe = function() {
        ...
        var array_img_id = [];
        $scope.dataImage.images_unpaid.map(img => {
            array_img_id.push(img.img_id);
        })
        
        NBDDataFactory.get('get_original_image', {img_ids: array_img_id}, function (response) {
            response = JSON.parse(response);
            if (response.status == 'success') {
                if (angular.isDefined(response.data) && response.data != '') {
                    $scope.dataDownload.downloadImageUrl = response.data;
                    stripe.createToken(cardNumberElement).then(function(result) {
                        if (result.error) {
                            ...
                            $scope.stripeTokenHandler(result.token, array_img_id);
                        }
                    });
                } else {
    ...
    $scope.stripeTokenHandler = function(token, array_img_id) {
        var data = {
            'stripeToken': token.id,
            'amount': $scope.dataImage.total * 100, //Convert to SGD
            'img_ids': array_img_id
        }

        NBDDataFactory.get('nbd_payment_stripe', data, function (response) {
            response = JSON.parse(response);
            if (response.status == 'success') {
                //handle success
                //Update image without watermark in canvas
                $scope.stages.map(_stage => {
                    var _canvas = _stage['canvas'];
                    response.data.map(img => {
                        //replace image url in object (will be save to design.json file
                        _canvas.forEachObject(function(obj, index) {
                            if (img.img_id === obj.get('img_id')) {
                                obj.setSrc(img.img_url);
                                obj.is_paid = true;
                            }
                        });
                        //replace image url in layer
                        _stage.layers.map(_layer => {
                            if (_layer.type === 'image') {
                                if (img.img_id === _layer.img_id) {
                                    _layer.src = img.img_url;
                                    _layer.is_paid = true;
                                }
                            }
                        })
                    })
                    if ($scope.resource.photo.type === "library") {
                        response.data.map(img => {
                            $scope.resource.photo.data.map(photo => {
                                if (img.img_id === photo.img_id) {
                                    photo.url = img.img_url;
                                    photo.src = img.img_url;
                                    photo.preview = img.img_url;
                                    photo.is_paid = true;
                                }
                            })
                        })
                ...
                $scope.dataImage.total = 0;
                $scope.dataImage.images_paid = $scope.dataImage.images_unpaid;
                $scope.dataImage.images_unpaid = [];
                $timeout(e => {
                    $scope.renderStage();
                }, 1000);
                $timeout(e => {
...
//                    $scope.saveData();
                ...
                //End update image without watermark in canvas
                if ($scope.dataDownload.downloadImageUrl != '' && typeof $scope.dataDownload.downloadImageUrl != "undefined") {
                    $scope.dataDownload.isPaid = true;
                } else {
    ...
    $scope.submitPaypalRequest = function() {
        if ($scope.dataDownload.downloadImageUrl != '' && typeof $scope.dataDownload.downloadImageUrl != "undefined") {
            var array_img_id = [];
            $scope.dataImage.images_unpaid.map(img => {
                array_img_id.push(img.img_id);
            ...
            var data = {
                'amount': $scope.dataImage.total, //Convert to SGD
                'img_ids': array_img_id
            }

            NBDDataFactory.get('nbd_paypal_request', data, function (response) {
                response = JSON.parse(response);
                if (response.status == 'success') {
                    //handle success
                    document.getElementById("nb-box-loading").style.display = "none";
                    var url = response.approval_link,
                    popupLeft = (window.screen.width - 700) / 2,
                    popupTop = (window.screen.height - 500) / 2;
                    var popup = window.open(url, '', 'width=700,height=500,left='+popupLeft+',top='+popupTop+'');
                    var home_url = NBDESIGNCONFIG['home_url'].substring(NBDESIGNCONFIG['home_url'].indexOf('//') + 2) //Get home url without http
                    var interval = setInterval(function () {
                        try {
                            if (popup) {
                                if (typeof popup.location.href !== 'undefined') {
                                    var success_url = home_url + '/payment-success/';
                                    var cancel_url = home_url + '/payment-cancel/';
                                    ...
                                    var popup_url = popup.location.host + popup.location.pathname; //Get url popup without http & parameter
                                    if (popup_url === cancel_url) {
                                        clearInterval(interval);
                                        popup.close();
                                    } else if (popup_url === success_url) {
                                        clearInterval(interval);
                                        popup.close();
                                        document.getElementById("nb-box-loading").style.display = "flex";
                                        ...
                                        var data = {
                                            'payerId': PayerID,
                                            'paymentId': paymentId,
                                            'amount': $scope.dataImage.total,
                                            'img_ids': array_img_id,
                                        }

                                        NBDDataFactory.get('nbd_payment_paypal', data, function (res) {
                                            res = JSON.parse(res);
                                            if (res.status == 'success') {
                                                ...
                                                //Empty $scope data Image, and update paid images
                                                $scope.dataImage.total = 0;
                                                $scope.dataImage.images_paid = $scope.dataImage.images_unpaid;
                                                $scope.dataImage.images_unpaid = [];
                                                $timeout(e => {
...
//                                                    $scope.saveData();
    ...
    //Download images after paid
    $scope.downloadImages = function() {
        if ($scope.dataDownload.downloadImageUrl != '' && typeof $scope.dataDownload.downloadImageUrl != "undefined") {
            var url = $scope.dataDownload.downloadImageUrl.replace(/['"]+/g, ''); //split " and ' character 
            window.location = window.atob(url); 
        }
    }
    $scope.hoverIn = function (event) {
        angular.element(event.currentTarget.parentElement).addClass('show');
    };
    $scope.hoverOut = function (event) {
        angular.element(event.currentTarget.parentElement).removeClass('show');
    };
    $scope.toggleShowImages = function(e) {
        jQuery("#nb-row-images").toggleClass("show");
    }
    ...
    $scope.generateStickerCutline = function( onSaveDesign ){
        ...
        bleedT = _stage.states.scaleRange[_stage.states.currentScaleIndex].ratio * _stage.config.bleed_t,
        bleedL = _stage.states.scaleRange[_stage.states.currentScaleIndex].ratio * _stage.config.bleed_l,
        bleedR = _stage.states.scaleRange[_stage.states.currentScaleIndex].ratio * _stage.config.bleed_r,
        bleedB = _stage.states.scaleRange[_stage.states.currentScaleIndex].ratio * _stage.config.bleed_b,
        margin = Math.ceil( Math.max( bleedT, bleedL, bleedR, bleedB ) );
        console.log(margin);
...
//            }
    ...
    if( NBDESIGNCONFIG.nbdesigner_enable_generate_photo_thumb == 'yes' ){
        fabric.Image.prototype._toSVG = function() {
            ...
            if( angular.isDefined( this.origin_url ) && !angular.isDefined( this.cropped ) ){
