...
<div id="nbdesign-order-tabs">
    <h2>
        ...
        <div id="customer-design">
            <div>
                ...
                <?php foreach( $resources as $resource ): ?>
                    ...
                    ?>
                    <tr>
                        <td><a href="<?php echo esc_url( $src ); ?>" target="_blank" title="<?php esc_html_e('View', 'web-to-print-online-designer') ?>"><?php esc_html_e( $src ); ?></a></td>
                    ...
                    <table class="nbd-resource-text">
                        ...
                        <?php foreach( $resources as $resource ): ?>
                            ...
                            <tr>
                                ...
                                <td style="font-family: <?php esc_attr_e( $alias ); ?>;"><?php esc_html_e( $layer->text ); ?></td>
                                <td>
                                    <?php if( gettype($layer->fill) == 'string' ): ?>
                                    <span class="nbd-color-wrap"><span class="nbd-color" style="background: <?php esc_attr_e( $layer->fill ); ?>"></span><span class="nbd-color-value"><?php esc_html_e( $layer->fill ); ?></span></span>
                                    <?php else: ?>
                                    <span>
                                ...
                                <td><?php esc_html_e( $fontSize ); ?></td>
                                <td><a href="<?php echo esc_url( $fonturl ); ?>" <?php if( $is_google_font ) echo 'target="_blank"'; else echo 'download'; ?> title="<?php esc_html_e('Download', 'web-to-print-online-designer'); ?>"><?php esc_html_e( $fontname ); ?> <span class="dashicons dashicons-external nbd-order-font-download"></span></a></td>
                            </tr>
                            <?php } endforeach; ?>
                        <?php endforeach; ?> 
                        </tbody>
        ...
        <div id="save-to-pdf">
                    ...
                    <tbody>
                        <tr valign="top">
                            ...
                            <td class="forminp forminp-text">
                                ...
                                <input name="order_id" type="hidden" value="<?php esc_attr_e( $_GET['order_id'] ); ?>">
                                <input name="nbd_item_key" type="hidden" value="<?php esc_attr_e( $_GET['nbd_item_key'] ); ?>">
                                <input type="hidden" value="<?php esc_attr_e( $option['dpi'] ); ?>" name="dpi">
            ...
            <div>
                ...
                <?php foreach($datas as $key => $data): ?>
                    <li><a href="#side-<?php esc_attr_e( $key ); ?>"><?php esc_html_e( $data['orientation_name'] ); ?></a></li>
                <?php endforeach; ?>
                </ul>
            </div>
        <?php
        ...
        ?>
        <div class="inner side" id="side-<?php esc_attr_e( $key ); ?>">
            <table class="form-table">
                <tbody>
                    <tr valign="top">
                        <th scope="row" class="titledesc">
                        ...
                        <td class="forminp forminp-text">
                            <input name="pdf[<?php esc_attr_e( $key ); ?>][name]" class="regular-text" type="text" value="<?php esc_attr_e( $data['orientation_name'] ); ?>">
                        </td>
                    </tr>
                    <tr valign="top">
                        <th scope="row" class="titledesc">
                        ...
                        <td class="forminp forminp-text">
                        <select name="pdf[<?php esc_attr_e( $key ); ?>][format]" id="format" class="regular-text">
                    ...
                    foreach ($output_format as $group=>$group_values) {
                        ?><optgroup label="<?php esc_attr_e( $group );?>"><?php
                            foreach ($group_values as $k => $val) {
                            ?>
                                <option value="<?php echo esc_attr($k); ?>"><?php esc_html_e( $val ); ?></option>
                            <?php
                            }
                        ?></optgroup><?php
                    }
                    ...
                    <tr valign="top">
                        ...
                        <td class="forminp forminp-text">
                            <?php esc_html_e('Width', 'web-to-print-online-designer'); ?>: <input name="pdf[<?php esc_attr_e( $key ); ?>][product-width]" type="text" readonly class="nbd-order-input-60" value="<?php esc_attr_e( $proWidth * $unitRatio ); ?>"> 
                            <?php esc_html_e('Hight', 'web-to-print-online-designer'); ?>: <input name="pdf[<?php esc_attr_e( $key ); ?>][product-height]" type="text" readonly class="nbd-order-input-60" value="<?php esc_attr_e( $proHeight * $unitRatio ); ?>"> 
                    ...
                    <tr valign="top">
                        ...
                        <td class="forminp forminp-text">
                            <select name="pdf[<?php esc_attr_e( $key ); ?>][orientation]" id="orientation">
                    ...
                    <tr valign="top">
                        ...
                        <td class="forminp forminp-text">
                            <input name="pdf[<?php esc_attr_e( $key ); ?>][margin-top]" class="margin-top-val nbd-order-input-60" type="number" min="0" value="0" >
                            <input name="pdf[<?php esc_attr_e( $key ); ?>][margin-right]" class="margin-right-val nbd-order-input-60" type="number" min="0" value="0" >
                            <input name="pdf[<?php esc_attr_e( $key ); ?>][margin-bottom]" class="margin-bottom-val nbd-order-input-60" type="number" min="0" value="0" >
                            <input name="pdf[<?php esc_attr_e( $key ); ?>][margin-left]" class="margin-left-val nbd-order-input-60" type="number" min="0" value="0" >
                    ...
                    <tr valign="top">
                        ...
                        <td class="forminp forminp-text">
                            <p>
<!--                                <input name="pdf[<?php //esc_attr_e( $key ); ?>][show-bleed-line]" type="radio" value="yes" onclick="showSettingBleed(this)">-->
                                <input name="pdf[<?php esc_attr_e( $key ); ?>][show-bleed-line]" type="radio" value="yes">
                                    <?php esc_html_e('Yes', 'web-to-print-online-designer'); ?>
                            </p>   
                            <p>
<!--                                <input name="pdf[<?php //esc_attr_e( $key ); ?>][show-bleed-line]" type="radio" value="no" checked="checked" onclick="showSettingBleed(this)">-->
                                <input name="pdf[<?php esc_attr_e( $key ); ?>][show-bleed-line]" type="radio" value="no" checked="checked">
                    ...
                    <tr valign="top" style="display: none;" class="setting-bleed">
                        ...
                        <td class="forminp forminp-text">
                            <input name="pdf[<?php esc_attr_e( $key ); ?>][bleed-top]" type="number" min="0" class="nbd-order-input-60" value="<?php esc_attr_e( $bleed_top_bottom ); ?>" onchange="changeBleed(this, 'top');">
                            <input name="pdf[<?php esc_attr_e( $key ); ?>][bleed-right]" type="number" min="0" class="nbd-order-input-60" value="<?php esc_attr_e( $bleed_left_right ); ?>" onchange="changeBleed(this, 'right');">
                            <input name="pdf[<?php esc_attr_e( $key ); ?>][bleed-bottom]" type="number" min="0" class="nbd-order-input-60" value="<?php esc_attr_e( $bleed_top_bottom ); ?>" onchange="changeBleed(this, 'bottom');">
                            <input name="pdf[<?php esc_attr_e( $key ); ?>][bleed-left]" type="number" min="0" class="nbd-order-input-60" value="<?php esc_attr_e( $bleed_left_right ); ?>" onchange="changeBleed(this, 'left');">
                    ...
                    <tr valign="top">
                        ...
                        <td class="forminp forminp-text">
                            ...
                            <span style="width: 30px; height: 30px; vertical-align: top; display: inline-block; background: <?php esc_attr_e( $data['bg_color_value'] ); ?>"></span>
                            <?php else: ?>
                    ...
                    <tr valign="top">
                        ...
                        <td class="forminp forminp-text">
                            <input name="pdf[<?php esc_attr_e( $key ); ?>][include_background]" type="hidden" value="0" />
                            <input name="pdf[<?php esc_attr_e( $key ); ?>][include_background]" type="checkbox" value="1" <?php checked( $data['include_background'] ); ?> />
            ...
            <div>
                <input type="hidden" value="<?php esc_attr_e( $data['img_src'] ); ?>" name="pdf[<?php esc_attr_e( $key ); ?>][background]" class="bg_src_hidden">
                <input type="hidden" value="<?php esc_attr_e( $data['bg_type'] ); ?>" name="pdf[<?php esc_attr_e( $key ); ?>][bg_type]">
				<input type="hidden" value="<?php esc_attr_e( $data['origin_bg_pdf'] ); ?>" name="pdf[<?php esc_attr_e( $key ); ?>][origin_bg_pdf]">
                <input type="hidden" value="<?php esc_attr_e( $data['bg_color_value'] ); ?>" name="pdf[<?php esc_attr_e( $key ); ?>][bg_color_value]">
                <input type="hidden" value="<?php esc_attr_e( $bgTop ); ?>" class="bg-top">
                <input type="hidden" value="<?php esc_attr_e( $bgLeft ); ?>" class="bg-left">
                <input type="hidden" value="<?php esc_attr_e( $bgWidth ); ?>" class="bg-width">
                <input type="hidden" value="<?php esc_attr_e( $bgHeight ); ?>" class="bg-height">
                <input type="hidden" value="<?php esc_attr_e( $cdTop ); ?>" class="cd-top" name="cd-top">
                <input type="hidden" value="<?php esc_attr_e( $cdLeft ); ?>" class="cd-left" name="cd-left">
                <input type="hidden" value="<?php esc_attr_e( $data['real_top'] * $unitRatio ); ?>" name="pdf[<?php esc_attr_e( $key ); ?>][cd-top]">
                <input type="hidden" value="<?php esc_attr_e( $data['real_left'] * $unitRatio ); ?>" name="pdf[<?php esc_attr_e( $key ); ?>][cd-left]"> 
                <input type="hidden" value="<?php esc_attr_e( $data['real_width'] * $unitRatio ); ?>" name="pdf[<?php esc_attr_e( $key ); ?>][cd-width]">
                <input type="hidden" value="<?php esc_attr_e( $data['real_height'] * $unitRatio ); ?>" name="pdf[<?php esc_attr_e( $key ); ?>][cd-height]">
                <input type="hidden" value="<?php esc_attr_e( $contentImage ); ?>" name="pdf[<?php esc_attr_e( $key ); ?>][customer-design]">
            </div>
        </div>
        <?php endforeach; ?>
        <p class="result-pdf">
