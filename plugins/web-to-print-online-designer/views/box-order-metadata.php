...
<div id="nbdesigner_order_info">
    <p><a href="javascript:void(0)" class="button" id="nbdesigner_delete_order_design"><?php esc_html_e('Delete order design','web-to-print-online-designer'); ?></a></p>
	<?php foreach($products AS $order_item_id => $product): ?>
        ...
        <div>
            ...
            <div class="nbdesigner_container_item_order <?php if(isset($data_designs[$index_accept])) { $status = ($data_designs[$index_accept] == 'accept') ? 'approved' : 'declined'; esc_attr_e( $status );}; ?>">
            <?php 
                $list_images = Nbdesigner_IO::get_list_images(NBDESIGNER_CUSTOMER_DIR .'/'. $nbd_item_key .'/preview', 1);  											
                if(count($list_images) > 0):
            ?>
                <input type="checkbox" name="_nbdesigner_design_file[]" class="nbdesigner_design_file" value="<?php esc_attr_e( $order_item_id ); ?>" />
            ...
            <div class="nbdesigner_container_item_order <?php if(isset($data_uploads[$index_accept])) { $status = ($data_uploads[$index_accept] == 'accept') ? 'approved' : 'declined'; esc_attr_e( $status );}; ?>">
                <input type="checkbox" name="_nbdesigner_upload_file[]" class="nbdesigner_design_file" value="<?php esc_attr_e( $order_item_id ); ?>" />
	...
	<input type="hidden" name="nbdesigner_design_order_id" value="<?php esc_attr_e( $order_id ); ?>" />
...
<div class="nbdesigner_container_order_email" id="nbdesigner_order_email_info">
	<h4><?php esc_html_e('Send mail','web-to-print-online-designer'); ?></h4>
	<?php wp_nonce_field('approve-design-email', '_nbdesigner_design_email_nonce'); ?>
	<input type="hidden" name="nbdesigner_design_email_order_id" value="<?php esc_attr_e( $order_id ); ?>" />
