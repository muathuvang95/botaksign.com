...
<div class="section-container">
    ...
    <div class="nbd-section-wrap">
        ...
        <?php //CS botak quantity break customer role?>
        <label>
        ...
        <div class="nbd-field-info nbd-field-default-quantity-break">
            <div class="nbd-table-wrap" style="overflow: hidden; margin-top: 5px;">
                <table class="nbd-table">
                    <tr>
        ...
        <div class="nbd-field-info nbd-field-quantity-break" ng-repeat="(role_index, role) in options.role_breaks">
            <div class="role-quantity-setup title" ng-show="!role.isExpand">{{role.role_name}}</div>
            <div class="role-quantity-setup" ng-show="role.isExpand">
                <div class="nbd-field-info-1">
                    <label>
                ...
                <div class="nbd-field-info-2">
                    <div class="nbd-table-wrap" style="overflow: hidden;">
                        <select name="options[role_breaks][{{role_index}}][role]" class="select-role" ng-change="changeRole(role_index)" ng-model="role.role">
                            <?php
                                global $wp_roles;
                        ...
                        <table class="nbd-table">
                            ...
                            <tr ng-repeat="break in options.role_breaks[role_index].quantity_breaks">
                                <input name="options[role_breaks][{{role_index}}][quantity_breaks][{{$index}}][val]" type="hidden" string-to-number class="nbd-short-ip" ng-model="break.val" ng-min="1" ng-value="{{options.quantity_breaks[$index].val}}"/>
                                <td><input name="options[quantity_breaks][{{$index}}][val]" type="number" string-to-number class="nbd-short-ip" ng-model="options.quantity_breaks[$index].val" ng-min="1"/></td>
                                <td><input name="options[role_breaks][{{role_index}}][quantity_breaks][{{$index}}][dis]" class="nbd-short-ip" type="text" ng-model="break.dis"/></td>
                                <td><input ng-checked="break.default" ng-click="role_update_default_quantity( role_index, $index )" name="options[role_breaks][{{role_index}}][quantity_breaks][{{$index}}][default]" type="checkbox" /></td>
                                <td><a class="button nbd-mini-btn"  ng-click="role_remove_price_break( role_index, $index )" title="<?php _e('Delete', 'web-to-print-online-designer'); ?>"><span class="dashicons dashicons-no-alt"></span></a></td>
                            </tr>
                        </table>
                    </div>
                    <div style="margin-top: 5px;">
                        <a class="button" ng-click="role_add_price_break(role_index)"><span class="dashicons dashicons-plus"></span> <?php _e('Add more', 'web-to-print-online-designer'); ?></a>
                    </div>
                </div>
            </div>
            <div class="nbd-field-info-3 field-action">
                <a class="nbd-field-btn nbd-mini-btn button" ng-click="deleteRoleQuantity(role_index)" title="<?php _e('Delete', 'web-to-print-online-designer'); ?>"><span class="dashicons dashicons-no-alt"></span></a>
                <a class="nbd-field-btn nbd-mini-btn button" ng-click="copyRoleQuantity(role_index)" title="<?php _e('Copy', 'web-to-print-online-designer'); ?>"><span class="dashicons dashicons-admin-page"></span></a>
                <a class="nbd-field-btn nbd-mini-btn button" ng-click="toggleRoleQuantity(role_index, $event)" title="<?php _e('Expand', 'web-to-print-online-designer'); ?>"><span ng-show="!role.isExpand" class="dashicons dashicons-arrow-down"></span><span ng-show="role.isExpand" class="dashicons dashicons-arrow-up"></span></a>
            </div>
        </div>
        <div style="display: inline-block; width: 100%;">
            <a class="button" ng-click="role_add_block()" style="float: right;"><span class="dashicons dashicons-plus"></span> <?php _e('Add role block', 'web-to-print-online-designer'); ?></a>
        </div>
    </div>
</div>