    ...
    <div class="nbd-nav">
        <div ng-dblclick="toggleExpandField($index, $event)" style="cursor: pointer;" title="<?php _e('Double click to expand option', 'web-to-print-online-designer') ?>">
            <ul nbd-tab ng-class="field.isExpand ? '' : 'left'" class="nbd-tab-nav">
                ...
                <?php //CS botak add calculation option ?>
                <li ng-if="field.nbd_type && field.nbd_type !== 'calculation_option' && field.nbd_type !== 'rounded_corner' && field.nbd_type !== 'terms_conditions'" class="nbd-field-tab" data-target="tab-online-design"><?php _e('Online design', 'web-to-print-online-designer'); ?></li>
                <li ng-if="field.nbd_type && field.nbd_type === 'rounded_corner'" ng-show="field.general.data_type.value =='m'" class="nbd-field-tab" data-target="tab-online-design"><?php _e('Online design', 'web-to-print-online-designer'); ?></li>
                <!-- custom botak Phase 4 material price -->
                <li ng-if="field.nbd_type && field.nbd_type === 'calculation_option'" class="nbd-field-tab" ng-click="get_data_sku(field , fieldIndex)" data-target="tab-online-design"><?php _e('Pricing', 'web-to-print-online-designer'); ?></li>
                <?php //End CS botak add calculation option ?>
...
<?php
    ...
    include 'templates/production_time.php'; //CS botak add custom option
    include 'templates/pricing_rates.php'; //CS botak add custom option
    include 'templates/calculation_option.php'; //CS botak add custom option
    include 'templates/terms_conditions.php'; //CS-V3 add new Terms & Conditions
    // -------------------------------------------------------------------------
    include 'templates/field-body/title.php';
    include 'templates/field-body/description.php';
    include 'templates/field-body/agreement_clause.php'; // Add new Field-body CS-V3 Agreement Clause
