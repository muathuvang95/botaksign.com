...
<h1><?php esc_html_e('Manager Fonts', 'web-to-print-online-designer'); ?></h1>
		...
		<form name="post" action="" method="post" enctype="multipart/form-data" autocomplete="off">
                <div class="nbdesigner-content-left postbox">
                    ...
                    <div class="inside nbdesigner_font_info">
                        ...
                        <table class="form-table">
                            <tr valign="top">
                                ...
                                <td class="forminp-text">
                                    <input id="font_name" class="nbdesigner_font_name" type="text" name="nbdesigner_font_name" placeholder="Henny Penny" value="<?php $name = isset($font_data->name) ?  $font_data->name : ''; esc_attr_e( $name ); ?>" />
                            ...
                            <tr valign="top">
                                ...
                                <td class="forminp-text">
                                    <input id="display_name" class="nbdesigner_font_name" type="text" name="display_name" value="<?php $display_name = isset($font_data->display_name) ?  $font_data->display_name : ''; esc_attr_e( $display_name ); ?>" />
                            ...
                            <tr valign="top">
                                ...
                                <td class="forminp-text">
                                    ...
                                    <div class="nbd-font-file-wrap">
                                        ...
                                        ?>
                                        <style type='text/css'>
                                            @font-face {font-family: <?php esc_attr_e( $font_data->alias ); ?>;src: local('☺'), url('<?php echo esc_url( $url ); ?>')}
                                        </style>
                                        <p style="font-family: <?php echo "'".$font_data->alias."', sans-serif"; ?>;font-size: 30px;">
                                            <a download href="<?php echo esc_url( $url ); ?>"><?php esc_html_e( $preview_text ); ?></a>
                                        </p>
                                        <?php endif; ?>
                                    </div>
                                    <div class="nbd-font-file-wrap">
                                        ...
                                        ?>
                                        <style type='text/css'>
                                            @font-face {font-family: <?php esc_attr_e( $font_data->alias ); ?>;src: local('☺'), url('<?php echo esc_url( $url ); ?>'); font-style: italic}
                                        </style>
                                    ...
                                    <div class="nbd-font-file-wrap">
                                        ...
                                        ?>
                                        <style type='text/css'>
                                            @font-face {font-family: <?php esc_attr_e( $font_data->alias ); ?>;src: local('☺'), url('<?php echo esc_url( $url ); ?>'); font-weight: bold}
                                        </style>
                                    ...
                                    <div class="nbd-font-file-wrap">
                                        ...
                                        ?>
                                        <style type='text/css'>
                                            @font-face {font-family: <?php esc_attr_e( $font_data->alias ); ?>;src: local('☺'), url('<?php echo esc_url( $url ); ?>'); font-style: italic; font-weight: bold}
                                        </style>
                        ...
                        <input type="hidden" name="nbdesigner_font_id" value="<?php esc_attr_e( $font_id ); ?>"/>
                        <p class="submit">
                            <input type="submit" name="Submit" class="button-primary" value="<?php esc_html_e('Save', 'web-to-print-online-designer'); ?>" />
                            <a href="?page=nbdesigner_manager_fonts" class="button-primary" style="<?php $style = (isset($_GET['id'])) ? '' : 'display:none;';esc_attr_e( $style ); ?>"><?php esc_html_e('Add New', 'web-to-print-online-designer'); ?></a>
                        </p>
                    </div>
                </div>
                        ...
                        <div class="inside">
                            ...
                            <?php if(is_array($cat) && (sizeof($cat) > 0)): ?>
                                <?php foreach($cat as $val): ?>
                                    <li id="nbdesigner_cat_font_<?php esc_attr_e( $val->id ); ?>" class="nbdesigner_action_delete_cf">
                                        <label>
                                            <input value="<?php esc_attr_e( $val->id ); ?>" type="checkbox" name="nbdesigner_font_cat[]" <?php if($update && (sizeof($cats) > 0 )) if(in_array($val->id, $cats)) echo "checked";  ?> />
                                        </label>
                                        ...
                                        <a href="<?php echo add_query_arg(array('cat_id' => $val->id), admin_url('admin.php?page=nbdesigner_manager_fonts')) ?>" class="nbdesigner-cat-link"><?php esc_html_e( $val->name ); ?></a>
                                        <input value="<?php esc_attr_e( $val->name ); ?>" class="nbdesigner-editcat-name" type="text"/>
                            ...
                            <input type="hidden" id="nbdesigner_current_font_cat_id" value="<?php esc_attr_e( $current_font_cat_id ); ?>"/>
	...
	<div class="postbox" id="nbd-list-fonts">
        <h3><?php  esc_html_e('List fonts ', 'web-to-print-online-designer'); ?>
            ...
            <select onchange="if (this.value) window.location.href=this.value+'#nbd-list-fonts'">
                ...
                <option value="<?php echo add_query_arg(array('cat_id' => $val->id), admin_url('admin.php?page=nbdesigner_manager_fonts')) ?>" <?php selected( $cat_index, $current_cat_id ); ?>><?php esc_html_e( $val->name ); ?></option>
                <?php endforeach; ?>
            </select>
            <?php  endif; ?>
                ...
                <?php if(is_array($list) && (sizeof($list) > 0)): ?>
                    <?php foreach($list as $val): ?>
                        <span class="nbdesigner_google_link ">
                            <a href="?page=nbdesigner_manager_fonts&id=<?php esc_attr_e( $val->id ); ?><?php if(isset($current_cat)) echo '&cat_id='.$current_cat; ?>">
                                <span><?php esc_html_e( $val->name ); ?></span>
                                <?php if( isset( $val->display_name ) && $val->display_name != $val->name ): ?>
                                    - <?php esc_html_e( $val->display_name ); ?>
                                <?php  endif; ?>
                            </a>
                            <span class="nbdesigner_action_delete_cfont" data-index="<?php esc_attr_e( $val->id ); ?>" onclick="NBDESIGNADMIN.delete_font('custom',this)">&times;</span>
                        </span>
                    <?php endforeach; ?>
                <?php else: ?>
