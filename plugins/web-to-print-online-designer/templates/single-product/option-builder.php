...
if( $options['display_type'] == 6 && isset( $options["popup_fields"] ) && count( $options["popup_fields"] ) ){
    ...
    foreach($options["popup_fields"] as $popup_field){
        $popup_field_index = get_field_by_id( $popup_field, $options["fields"] );
...
if ($_designer_setting /*&& $enable*/) {
    ...
    if ($_designer_setting) {
        ...
        $product_size['real_width'] = $first_side['real_width'];
        $product_size['real_height'] = $first_side['real_height'];
        $product_size['real_top'] = $first_side['real_top'];
        $product_size['real_left'] = $first_side['real_left'];
    }
}

//CS botak Get tax rate of product to calc price with tax after change quantity
global $product;
$tax_rate = 0;
...
foreach( $options["fields"] as $key => $field ){
    ...
    if( $field['general']['data_type'] == 'i' ){
        ...
        if( isset($field['nbd_type']) && ( ( $field['nbd_type'] == 'page' && $field['general']['data_type'] == 'm' ) || ( $field['nbd_type'] == 'page2' ) ) ){
            // Print option: Add option Dropdown & Advanced Dropdown for Sides Pages
        ...
        if( isset($field['nbd_type']) && $field['nbd_type'] == 'production_time') {
            switch($field['appearance']['display_type']){
                case 's':
                    $tempalte = $currentDir .'/options-builder//options-production-time/swatch.php';
                    break;
                case 'l':
                    $tempalte = $currentDir .'/options-builder/options-production-time/label.php';
                    break;
                case 'r':
                    $tempalte = $currentDir .'/options-builder/options-production-time/radio.php';
                    break;
                case 'ad':
                     $tempalte = $currentDir .'/options-builder/options-production-time/advanced-dropdown.php';
                    break;
                case 'xl':
                    $tempalte = $currentDir .'/options-builder/options-production-time/xlabel.php';
                    break;
                default:
                    $tempalte = $currentDir .'/options-builder/options-production-time/advanced-dropdown.php';
                    break;
            }
            if( count($field['general']['role_options']) != 0 ) {
                $need_show = true;
            }
        }
    }
...
<script type="text/javascript">
    ...
    function ajax_change_product(cur_id , product_id) {
        var product_id = jQuery('.merged_product_ajax').val();
        var cur_id = jQuery('.nbd_merged_product input').val();
        jQuery.ajax({
            type : "post",
            dataType : "json",
            url : '<?php echo admin_url('admin-ajax.php');?>',
            data : {
                    action: "get_content_product",
                    product_id : product_id,
                    cur_id : cur_id,
            },
            context: this,
            beforeSend: function() {
                jQuery('.nbd-loader-ajax').css('display' , 'block');
            },
            complete: function() {
                jQuery('.nbd-loader-ajax').css('display' , 'none');
            },
            success: function(response) {
                if(response.success) {
                    var product = jQuery(response.data['template_single_product']).find('.single-product-wrap').children();
                    var tab = jQuery(response.data['template_single_product']).find('.woocommerce-tabs').children();
                    var related = jQuery(response.data['template_single_product']).find('.section.related').children();
                   jQuery('main.shop-main .single-product-wrap').html(product);
                   jQuery('main.shop-main .woocommerce-tabs').html(tab);
                   jQuery('main.shop-main .section.related').html(related);
                   jQuery('.product_meta').after(response.data['selected']);
                   jQuery('h1.product_title').html(response.data['title']);
                   jQuery('.nbd_merged_product input').val(cur_id);
                   jQuery('form.cart').attr('action' , response.data['link']);
                   jQuery('form.cart .single_add_to_cart_button').css('display' , 'inline-block');
                   jQuery('.nb-quantity .qty-buttons .quantity-plus-cart').attr('onclick' , 'plus_val_qty()');
                   jQuery('.nb-quantity .qty-buttons .quantity-minus-cart').attr('onclick' , 'minus_val_qty()');
                   var button = jQuery(response.data['template_single_product']).find('.single_add_to_cart_button');
                   jQuery('.price del').remove();
                   jQuery('form.cart').attr('id' , 'add_to_cart_outside');
                   jQuery('.entry-summary>button.single_add_to_cart_button').attr('form' , 'add_to_cart_outside');
                   jQuery('.merged_product_ajax option').each(function() {
                        if(jQuery(this).val() == product_id) {
                            jQuery(this).attr("selected","selected");
                        }
                   });
                   jQuery('.nbdesigner_frontend_container').before(response.data['script_nbdesigner']);
                   jQuery('.product-image .woocommerce-product-gallery').before(response.data['swiper']);
                }
            },
    ...
    function plus_val_qty() {
        var d = jQuery('.nb-quantity>input').val();
        d++;
        jQuery('.nb-quantity>input').val(d);
        var scope = angular.element(document.getElementById(nbOption.crtlId)).scope();
        if(scope) {
            scope.check_valid();
        }
    }
    function minus_val_qty() {
        var d = jQuery('.nb-quantity>input').val();
        if(d>1) {
            d--;
        }
    ...
    nboApp.controller('optionCtrl', ['$scope', '$timeout', function($scope, $timeout){
        ...
        $scope.user_role = "<?php echo $user_role; ?>";
        $scope.check_valid = function( calculate_pm, pro ){
            //CS botak button add to card
            if (angular.isDefined($scope.nbd_fields) && angular.equals($scope.nbd_fields, {})) {
                jQuery('button[type="submit"].single_add_to_cart_button').show();
            } else if (!angular.isDefined($scope.nbd_fields)) {
                jQuery('button[type="submit"].single_add_to_cart_button').show();
            }
            $timeout(function(){
                ...
                angular.forEach($scope.nbd_fields, function(field, field_id){
                    ...
                    if(origin_field.nbd_type == "page2") {
                        switch($scope.nbd_fields[field_id].value) {
                            case '0':
                                $scope.nbd_fields[field_id].values = [ 0 ];
                                break;
                            case '1':
                                $scope.nbd_fields[field_id].values = [ 0 , 1 ];
                                break;
                            case '2':
                                $scope.nbd_fields[field_id].values = [ 0 , 1 , 2];
                                break;
                            case '3':
                                $scope.nbd_fields[field_id].values = [ 0 , 1 , 2 , 3];
                                break;
                          default:
                            $scope.nbd_fields[field_id].values = [ 0 ];
                        }
                    ...
                    //CS botak calc price with production time option V3
                    var qty = $scope.validate_int(jQuery('input[name="quantity"]').val());
                    if(qty == 0) {
                         qty = 1;
                    }
                    var options_production_time = [];
                    if(origin_field.nbd_type == "production_time" && $scope.nbd_fields[field_id].enable) {
                        $scope.time_work = [];
                        var time_work = 0;
                        var have_role_use = false;
                        var have_check_default = false;
                        angular.forEach(origin_field.general.role_options , function( role_options , key) {
                            if( angular.isDefined(role_options.options[field.value]) && $scope.user_role == role_options.role_name ) {
                                value_name_1 = role_options.options[field.value].name
                                options_production_time_1 = role_options;
                                have_role_use = true;
                            } 
                            if( angular.isDefined(role_options.options[field.value]) && ( role_options.check_default == 'on' ||  role_options.check_default == '1' ))  {
                                value_name_2 = role_options.options[field.value].name
                                options_production_time_2 = role_options;
                                have_check_default = true;
                            }
                        });
                        if(have_role_use) {
                            field.value_name = value_name_1;
                            options_production_time = options_production_time_1;
                        } else {
                            if( have_check_default ) {
                                field.value_name = value_name_2;
                                options_production_time = options_production_time_2;
                            }
                        }
                        if(angular.isDefined(options_production_time)) {
                            angular.forEach(options_production_time.options , function(options , k) {
                                var length_br = options.time_quantity_breaks.length;
                                for( var i =0; i < length_br; i++) {
                                    if(qty >= options.time_quantity_breaks[length_br-1].qty ) {
                                        time_work = options.time_quantity_breaks[length_br-1].time;
                                    }
                                    else if( qty >= options.time_quantity_breaks[i].qty && qty < options.time_quantity_breaks[i+1].qty ) {
                                        time_work = options.time_quantity_breaks[i].time;
                                    }
                                }
                                $scope.time_work[k] = time_work;
                            });
                        }
                    }
                    //End CS botak calc price with production time option V3

                    // CS botak Fix price option old of production time
                    if (angular.isDefined(origin_field.nbd_type) && origin_field.nbd_type == 'production_time') {
                        if(origin_field.general.attributes.options.length > 0 && angular.isDefined(origin_field.general.attributes.options[field.value].price) ) {
                            origin_field.general.attributes.options[field.value].price = 0;
                        }
                    }
                    // End
                    ...
                    if( origin_field.general.data_type == 'i' ){
                        ...
                        if( angular.isDefined(field.values) ){
                            ...
                            //CS botak hidden invalid option
                           if( origin_field.general.attributes.options.length ){
                                ...
                                if (field_id_map != '' && !$scope.status_fields[field_id][field.value].enable) {
                                    ...
                                    angular.forEach( $scope.status_fields[field_id] , function(value , key ) {
                                        if(value.enable) {
                                            select = key.toString();
                                            selected_option = origin_field.general.attributes.options[select];
                                        }
                                    })
                                    field.value = select;
                                    field.value_name = origin_field.general.attributes.options[select].name;
                                }
                                //Hidden invalid option of advanced dropdown option
                                if( angular.isDefined( field.sub_value ) ){
                                    if( angular.isDefined( selected_option.enable_subattr ) && selected_option.enable_subattr == 'on' && selected_option.sub_attributes.length > 0 ){
                                        if( !$scope.status_fields[field_id][field.value].sub_attributes[field.sub_value] ){
                                            // $scope.status_fields[field_id][field.value].sub_attributes
                                            let sub_select = '0';
                                            selected_option.sub_attributes.map((sa, su_index) => {
                                                if (su_index.toString() !== field.sub_value.toString()) {
                                                    sub_select = su_index.toString()
                                                }
                                            });
                                            $scope.nbd_fields[field_id].sub_value = selected_option.sub_attributes[sub_select];
                                        }
                                    }
                                }
                            }
                        }else{
                            if(angular.isDefined(origin_field.nbd_type) && origin_field.nbd_type == 'production_time' && options_production_time.length > 0) {
                                var selected_option = options_production_time.options[field.value];
                            } else {
                            ...
                            if( origin_field.general.attributes.options.length ){
                                ...
                                //Hidden invalid option of dropdown option
                                // if (!$scope.status_fields[field_id][field.value].enable) {
                                //     let select = '0';
                                //     origin_field.general.attributes.options.map((op, index) => {
                                //         if (index.toString() !== field.value.toString()) {
                                //             select = index.toString();
                                //             selected_option = origin_field.general.attributes.options[index];
                                //         }
                                //     })
                                //     field.value = select;
                                // }
                            ...
                            //End CS botak hidden invalid option
                            ...
                            if( origin_field.nbe_type == 'actions' ){
                                ...
                                if( !( action == 'u' || action == 'c' ) ){
                                    ...
                                    if( jQuery('#triggerDesign').length > 0 && angular.isDefined( nbds_frontend ) && nbds_frontend.hide_cart_button == 'yes'){
                                        jQuery('button[type="submit"].single_add_to_cart_button').show();
                                    };
                                }else{
                                    jQuery('#triggerDesign').show();
                    ...
                    var checkAgree = jQuery('input[type="checkbox"].agreement_clause_check');
                    if (origin_field.nbd_type == "terms_conditions") {
                        if (checkAgree.prop("checked") == true && field.enable == true) {
                            jQuery('.single-product .single-product-wrap .nbdesigner_frontend_container').removeClass('open');
                        } else if(field.enable == true){
                            jQuery('.single-product .single-product-wrap .nbdesigner_frontend_container').addClass('open');
                            jQuery('.single_add_to_cart_button').attr('disabled', true);
                            jQuery('.single_add_to_cart_button').addClass('btn_disabled');
                        } else if(field.enable == false){
                            jQuery('.single-product .single-product-wrap .nbdesigner_frontend_container').removeClass('open');
                            jQuery('.single_add_to_cart_button').attr('disabled', false);
                            jQuery('.single_add_to_cart_button').removeClass('btn_disabled');
                        }
                    }
                ...
                /* //CS botak gallery options
                    $scope.may_be_change_product_image();
                */
                $scope.change_product_image();
                ...
                //CS botak SKU product
                $scope.change_product_sku();
                $scope.update_app();
            });
        };
        //CS botak SKU product
        $scope.change_product_sku = function() {
            setTimeout(function(){0<jQuery(".nbo-invalid-option.active").length&&jQuery(".nbo-invalid-option.active").siblings("select").val(0).change()},300);
            var sku = '';
            sku = $scope.get_gallery_sku();
            angular.forEach($scope.nbd_fields, function(field, field_id){
                var origin_field = $scope.get_field(field_id);
                if( origin_field.general.data_type == 'm' && origin_field.nbd_type !== 'production_time'){
                    var field_sku = origin_field.general.attributes.options[field.value].sku;
                    if (typeof field_sku !== 'undefined' && field_sku != '' && field.enable) {
                        sku += field_sku;
                    }
                }
            });
            ...
            if (sku !== '') {
                jQuery('.product_meta .sku_wrapper .sku').html(sku);
            } else {
                jQuery('.product_meta .sku_wrapper .sku').html('');
            }
        }
        //CS botak get gallery SKU
        $scope.get_gallery_sku = function () {
            //CS botak check condition to chang gallery
            for (var i = 0; i < $scope.options.gallery_options.length; i++) {
                var change = false,
                    checks = [],
                    option = $scope.options.gallery_options[i];
                if( angular.isDefined( option.gallery_depend ) && angular.isDefined( option.gallery_enable_con ) ){
                    if( option.gallery_enable_con === 'on' && option.gallery_depend.length > 0 ){
                        var logic = option.gallery_con_logic,
                            total_check = logic == 'a' ? true : false;
                        angular.forEach(option.gallery_depend, function(con, key){
                            if( con.id != '' ){
                ...
                if (option.gallery_enable_con === 'off' || typeof option.gallery_enable_con === 'undefined') {
                    change = true;
                }

                if (change === true) {
                    var sku = '';
                    if (typeof option.sku !== 'undefined') {
                        sku = option.sku
                    }
                    return sku;
                }
                    ...
                    if( angular.isDefined(origin_field.nbd_type) ){
                        switch(origin_field.nbd_type){
                            ...
                            case 'page2':
                                ...
                                nbOption.odOption.page.list_page = list_page;
                                break;
                            case 'page3':
                                var list_page = [0, 1];
                                if( field.value == 0 ){
                                ...
                                break;
                            case 'size':
                                // CS Botak V3 Bleed & Safe Zone Corner
                                if(origin_field.general.attributes.same_size == 'n'){
                                    ...
                                    nbOption.odOption.size = {
                                        ...
                                        //CS botak option size
                                        show_bleed: option_size.show_bleed == 'on' ? '1' : '0',
                                        // CS-V3 add new Bleed
                                        bleed_top: option_size.show_bleed == 'on' ? $scope.validate_float( option_size.bleed_top ) : 0,
                                        bleed_bottom: option_size.show_bleed == 'on' ? $scope.validate_float( option_size.bleed_bottom ) : 0,
                                        bleed_left: option_size.show_bleed == 'on' ? $scope.validate_float( option_size.bleed_left ) : 0,
                                        bleed_right: option_size.show_bleed == 'on' ? $scope.validate_float( option_size.bleed_right) : 0,
                                        show_safe_zone: option_size.show_safe_zone == 'on' ? '1' : '0',
                                        // CS-v3 add new Safe of zone
                                        margin_top: option_size.show_safe_zone == 'on' ? $scope.validate_float( option_size.margin_top ) : 0,
                                        margin_left: option_size.show_safe_zone == 'on' ? $scope.validate_float( option_size.margin_left ) : 0,
                                        margin_bottom: option_size.show_safe_zone == 'on' ? $scope.validate_float( option_size.margin_bottom ) : 0,
                                        margin_right: option_size.show_safe_zone == 'on' ? $scope.validate_float( option_size.margin_right ) : 0,
                                        show_overlay: option_size.show_overlay == 'on' ? '1' : '0',
                                        // CS-V3 Bleed line , Safe corner radius
                                        bleed_corner_radius: option_size.bleed_corner_radius == 'on' ? '1' : '0',
                                        
                                        bleed_corner_top: option_size.bleed_corner_radius == 'on' ? $scope.validate_float( option_size.bleed_corner_top ) : $scope.validate_float( option_size.bleed_corner_top ),
                                        bleed_corner_left: option_size.bleed_corner_radius == 'on' ? $scope.validate_float( option_size.bleed_corner_left ) : $scope.validate_float( option_size.bleed_corner_left ),

                                        bleed_corner_bottom: option_size.bleed_corner_radius == 'on' ? $scope.validate_float( option_size.bleed_corner_bottom ) : $scope.validate_float( option_size.bleed_corner_bottom ),
                                        bleed_corner_right: option_size.bleed_corner_radius == 'on' ? $scope.validate_float( option_size.bleed_corner_right ) : $scope.validate_float( option_size.bleed_corner_right ),

                                        
                                        safe_corner_radius: option_size.safe_corner_radius == 'on' ? '1' : '0',
                                        safe_corner_margin_top: option_size.safe_corner_radius == 'on' ? $scope.validate_float( option_size.safe_corner_margin_top ) : $scope.validate_float( option_size.safe_corner_margin_top ),
                                        safe_corner_margin_left: option_size.safe_corner_radius == 'on' ? $scope.validate_float( option_size.safe_corner_margin_left ) : $scope.validate_float( option_size.safe_corner_margin_left ),
                                        safe_corner_margin_bottom: option_size.safe_corner_radius == 'on' ? $scope.validate_float( option_size.safe_corner_margin_bottom ) : $scope.validate_float( option_size.safe_corner_margin_bottom ),
                                        safe_corner_margin_right: option_size.safe_corner_radius == 'on' ? $scope.validate_float( option_size.safe_corner_margin_right ) : $scope.validate_float( option_size.safe_corner_margin_right ),
                                    };
                                    if (angular.isDefined(option_size.img_overlay)) {
                                        nbOption.odOption.size.img_overlay = option_size.img_overlay;
                                    }
                                } else {
                                //CS botak option size (bleed zone, overlay,...)
                                    var option_size = origin_field.general.attributes.options[field.value];
                                    nbOption.odOption.size = {
                                        product_width: $scope.validate_float( <?php echo $product_size['product_width']; ?> ),
                                        product_height: $scope.validate_float( <?php echo $product_size['product_height']; ?>  ),
                                        real_width: $scope.validate_float( <?php echo $product_size['real_width']; ?>  ),
                                        real_height: $scope.validate_float( <?php echo $product_size['real_height']; ?>  ),
                                        real_top: $scope.validate_float( <?php echo $product_size['real_top']; ?>  ),
                                        real_left: $scope.validate_float( <?php echo $product_size['real_left']; ?>  ),
                                        show_bleed: option_size.show_bleed == 'on' ? '1' : '0',
                                        // CS-V3 add new Bleed                             
                                        bleed_top: option_size.show_bleed == 'on' ? $scope.validate_float( option_size.bleed_top ) : 0,
                                        bleed_bottom: option_size.show_bleed == 'on' ? $scope.validate_float( option_size.bleed_bottom ) : 0,
                            ...
                            case 'rounded_corner':
                                var option = origin_field.general.attributes.options[field.value];
                                // CS-V3 Check rounded_corner
                                if(angular.isDefined(option)){
                                    nbOption.extraOdOption.rounded_corner = parseFloat(option.radius);
                                }
                                else {
                                   nbOption.extraOdOption.rounded_corner =  parseFloat(field.value);
                                }
                                break;
                            case 'overlay':
                                var option = origin_field.general.attributes.options[field.value];
        ...
        //CS botak change image gallery option, get item of slide gallery in product page
        $scope.product_featured_galley_items = jQuery( '#product-'+ $scope.product_id + ' .featured-gallery .swiper-slide' );
        $scope.product_thumb_galley_items = jQuery( '#product-'+ $scope.product_id + ' .thumb-gallery .swiper-slide' );
        
        //CS botak change image gallery option, get item of slide gallery in product page
        $scope.change_product_image = function(){
            var product_element = jQuery( '#product-'+ $scope.product_id );
            //CS botak change product gallery image
            ...
            var product_featured_swiper_wrapper = product_element.find( '.featured-gallery .swiper-wrapper' );
            var product_gallery_swiper_wrapper = product_element.find( '.thumb-gallery .swiper-wrapper' );
            product_featured_swiper_wrapper.empty();
            product_gallery_swiper_wrapper.empty();
            ...
            for (var i = 0; i < $scope.options.gallery_options.length; i++) {
                ...
                if( angular.isDefined( option.gallery_depend ) && angular.isDefined( option.gallery_enable_con ) ){
                    if( option.gallery_enable_con === 'on' && option.gallery_depend.length > 0 ){
                        ...
                        angular.forEach(option.gallery_depend, function(con, key){
                            if( con.id != '' ){
//                                console.log(!$scope.nbd_fields[con.id]);
//                                console.log(!$scope.nbd_fields[con.id].enable);
                                if(  con.id != 'qty' && ( angular.isUndefined($scope.nbd_fields[con.id]) || !$scope.nbd_fields[con.id].enable ) ){
                                    checks[key] = false;
                                }else{
                                    if( con.id == 'qty' ){
                ...
                if (change === true && typeof option.product_images !== 'undefined') {
                    if (option.product_images.length) {
                        option.product_images.map(img_data => {
                            var item_feature_image = jQuery('<img class="attachment-shop_single size-shop_single wp-post-image"/>');
                            var gallery_item_feature_image = jQuery('<img class="attachment-medium size-medium wp-post-image"/>');

                            if( !img_data.full_src_w ) img_data.full_src = item_feature_image.attr('data-large_image_width');
                            if( !img_data.full_src_h ) img_data.full_src_h = item_feature_image.attr('data-large_image_height');
                            $scope.set_product_image_attr(item_feature_image, 'src', img_data.image_link, 0);
                            $scope.set_product_image_attr(item_feature_image, 'srcset', img_data.image_srcset, 0);
                            $scope.set_product_image_attr(item_feature_image, 'sizes', img_data.image_sizes, 0);
                            $scope.set_product_image_attr(item_feature_image, 'title', img_data.image_title, 0);
                            $scope.set_product_image_attr(item_feature_image, 'alt', img_data.image_alt, 0);
                            $scope.set_product_image_attr(item_feature_image, 'data-src', img_data.full_src, 0);
                            $scope.set_product_image_attr(item_feature_image, 'data-large_image', img_data.full_src, 0);
                            $scope.set_product_image_attr(item_feature_image, 'data-large_image_width', img_data.full_src_w, 0);
                            $scope.set_product_image_attr(item_feature_image, 'data-large_image_height', img_data.full_src_h, 0);

                            if( !img_data.full_src_w ) img_data.full_src = gallery_item_feature_image.attr('data-large_image_width');
                            if( !img_data.full_src_h ) img_data.full_src_h = gallery_item_feature_image.attr('data-large_image_height');
                            $scope.set_product_image_attr(gallery_item_feature_image, 'src', img_data.image_link, 0);
                            $scope.set_product_image_attr(gallery_item_feature_image, 'srcset', img_data.image_srcset, 0);
                            $scope.set_product_image_attr(gallery_item_feature_image, 'sizes', img_data.image_sizes, 0);
                            $scope.set_product_image_attr(gallery_item_feature_image, 'title', img_data.image_title, 0);
                            $scope.set_product_image_attr(gallery_item_feature_image, 'alt', img_data.image_alt, 0);
                            $scope.set_product_image_attr(gallery_item_feature_image, 'data-src', img_data.full_src, 0);
                            $scope.set_product_image_attr(gallery_item_feature_image, 'data-large_image', img_data.full_src, 0);
                            $scope.set_product_image_attr(gallery_item_feature_image, 'data-large_image_width', img_data.full_src_w, 0);
                            $scope.set_product_image_attr(gallery_item_feature_image, 'data-large_image_height', img_data.full_src_h, 0);

                            var item_feature = jQuery('<div class="woocommerce-product-gallery__image swiper-slide"></div>').append(item_feature_image);
                            var gallery_item_slide = jQuery('<div class="woocommerce-product-gallery__image swiper-slide"></div>').append(gallery_item_feature_image);
                            product_featured_swiper_wrapper.append(item_feature);
                            product_gallery_swiper_wrapper.append(gallery_item_slide);
                        })

                        //dispatchEvent resize windown to resize swipe slide
            ...
            //Set default gallery if condition not fit
            var product_featured_galley_items = $scope.product_featured_galley_items;
            var product_thumb_galley_items = $scope.product_thumb_galley_items;
            if (product_featured_galley_items.length) {
                product_featured_galley_items.map((index, fe) => {
                    if (index === 0 && fe.innerHTML === '') {
                        var img_tmp = jQuery('<img class="attachment-medium size-medium wp-post-image"/>');
                        if (product_thumb_galley_items.length > 0) {
                            //Set first img for slide gallery
                            var img_first = jQuery(product_thumb_galley_items[0]).find('img')[0];
                            if (typeof img_first !== 'undefined') {
                                $scope.set_product_image_attr(img_tmp, 'src', img_first.img_first, 0);
                                $scope.set_product_image_attr(img_tmp, 'srcset', img_first.srcset, 0);
                                $scope.set_product_image_attr(img_tmp, 'title', img_first.title, 0);
                                $scope.set_product_image_attr(img_tmp, 'alt', img_first.alt, 0);
                                $scope.set_product_image_attr(img_tmp, 'width', 600, 0);
                                $scope.set_product_image_attr(img_tmp, 'height', 600, 0);
                                fe.append(img_tmp[0]);
                            }
                        }
                    }
                    product_featured_swiper_wrapper.append(fe);
                })
            } else {
                //Set first img for slide gallery
                let fe = jQuery('<div class=".woocommerce-product-gallery__image swiper-slide"></div>');
                let img_tmp = jQuery('<img class="attachment-medium size-medium wp-post-image"/>');
                $scope.set_product_image_attr(img_tmp, 'src', '<?php echo wc_placeholder_img_src();?>', 0);
                $scope.set_product_image_attr(img_tmp, 'width', 600, 0);
                $scope.set_product_image_attr(img_tmp, 'height', 600, 0);
                fe.append(img_tmp[0]);
                product_featured_swiper_wrapper.append(fe);
                
                let te = jQuery('<div class="woocommerce-product-gallery__image swiper-slide"></div>');
                let img_tmp_2 = jQuery('<img class="attachment-medium size-medium wp-post-image"/>');
                //Set first img for slide gallery
                $scope.set_product_image_attr(img_tmp_2, 'src', '<?php echo wc_placeholder_img_src();?>', 0);
                $scope.set_product_image_attr(img_tmp_2, 'width', 183, 0);
                $scope.set_product_image_attr(img_tmp_2, 'height', 183, 0);
                te.append(img_tmp_2[0]);
                product_gallery_swiper_wrapper.append(te);
            }
            product_thumb_galley_items.map((index, te) => {
                product_gallery_swiper_wrapper.append(te);
            })
            //dispatchEvent resize windown to resize swipe slide
            window.dispatchEvent(new Event('resize'));
        ...
        /* //Function change product image old
        $scope.change_product_image = function( field_id, option_index ){
            ...
            if( field.appearance.change_image_product == 'y' && field.general.attributes.options[option_index].imagep == 'y' ){
                ...
                var change = true,
                    checks = [],
                    option = field.general.attributes.options[option_index];
                if( angular.isDefined( option.gallery_depend ) && angular.isDefined( option.gallery_enable_con ) ){
                    if( option.gallery_enable_con === 'on' && option.gallery_depend.length > 0 ){
                        var logic = option.gallery_con_logic,
                            total_check = logic == 'a' ? true : false;
                ...
                if (product_image.length && change === true){
                    ...
                    var product_item_slide = jQuery('<div class="woocommerce-product-gallery__image swiper-slide"></div>').append(product_image);
                    product_featured_swiper_wrapper.append(product_item_slide);

                    var gallery_item_feature_image_first = jQuery('<img class="attachment-medium size-medium wp-post-image"/>');    
                    if( !option_data.full_src_w ) option_data.full_src = gallery_item_feature_image_first.attr('data-large_image_width');
                    if( !option_data.full_src_h ) option_data.full_src_h = gallery_item_feature_image_first.attr('data-large_image_height');
                    $scope.set_product_image_attr(gallery_item_feature_image_first, 'src', option_data.image_link, 0);
                    $scope.set_product_image_attr(gallery_item_feature_image_first, 'srcset', option_data.image_srcset, 0);
                    $scope.set_product_image_attr(gallery_item_feature_image_first, 'sizes', option_data.image_sizes, 0);
                    $scope.set_product_image_attr(gallery_item_feature_image_first, 'title', option_data.image_title, 0);
                    $scope.set_product_image_attr(gallery_item_feature_image_first, 'alt', option_data.image_alt, 0);
                    $scope.set_product_image_attr(gallery_item_feature_image_first, 'data-src', option_data.full_src, 0);
                    $scope.set_product_image_attr(gallery_item_feature_image_first, 'data-large_image', option_data.full_src, 0);
                    $scope.set_product_image_attr(gallery_item_feature_image_first, 'data-large_image_width', option_data.full_src_w, 0);
                    $scope.set_product_image_attr(gallery_item_feature_image_first, 'data-large_image_height', option_data.full_src_h, 0);
                    var product_gallery_item_slide = jQuery('<div class="woocommerce-product-gallery__image swiper-slide"></div>').append(gallery_item_feature_image_first);
                    product_gallery_swiper_wrapper.append(product_gallery_item_slide);
                }

                if (typeof option_data.product_images !== 'undefined' && option_data.product_images.length > 0 && change === true) {
                    option_data.product_images.map(img_data => {
                        var item_feature_image = jQuery('<img class="attachment-shop_single size-shop_single wp-post-image"/>');
                        var gallery_item_feature_image = jQuery('<img class="attachment-medium size-medium wp-post-image"/>');
                    ...
                    product_featured_galley_items.map((index, fe) => {
                        if (index === 0 && fe.innerHTML === '') {
                            ...
                            if (product_thumb_galley_items.length > 0) {
                                ...
                                $scope.set_product_image_attr(img_tmp, 'src', option_data.image_link, 0);
                                $scope.set_product_image_attr(img_tmp, 'srcset', option_data.image_srcset, 0);
                                $scope.set_product_image_attr(img_tmp, 'sizes', option_data.image_sizes, 0);
                                $scope.set_product_image_attr(img_tmp, 'title', option_data.image_title, 0);
                                $scope.set_product_image_attr(img_tmp, 'alt', option_data.image_alt, 0);
                                $scope.set_product_image_attr(img_tmp, 'width', 600, 0);
                                $scope.set_product_image_attr(img_tmp, 'height', 600, 0);
                                $scope.set_product_image_attr(img_tmp, 'data-src', option_data.full_src, 0);
                                $scope.set_product_image_attr(img_tmp, 'data-large_image', option_data.full_src, 0);
                                $scope.set_product_image_attr(img_tmp, 'data-large_image_width', option_data.full_src_w, 0);
                                $scope.set_product_image_attr(img_tmp, 'data-large_image_height', option_data.full_src_h, 0);
                            }
                        }
                        product_featured_swiper_wrapper.append(fe);
                    })
        ...
        */
        ...
        $scope.init = function(){
            ...
            angular.forEach($scope.form_values, function(value, field_id){
                if(Object.keys($scope.form_values)[0] == Object.keys($scope.nbd_fields)[0]) {
                    if(field_id){
        ...
        $scope.reset_options = function(){
            ...
            $scope.quantity = $scope.validate_int("<?php echo $options['role_breaks'][0]['quantity_breaks'][0]['val']; ?>");
            <?php endif; ?>
                    ...
                    angular.forEach(nbd_fields, function(field, field_id){
                        if(field.enable){
                            ...
                            if( origin_field.general.data_type == 'i' ){
                                ...
                                if( angular.isDefined(origin_field.nbd_type) && origin_field.nbd_type == 'dimension' 
                                        && origin_field.general.measure == 'y' /*&& angular.isDefined(origin_field.general.measure_range) && origin_field.general.measure_range.length > 0 */ ){ //CS botak dimention multi component
                                    ...
                                    if( (origin_field.general.price_type == 'f' || origin_field.general.price_type == 'c')
                                            && origin_field.general.measure_base_pages == 'y' ){
                            ...
                            if( $scope.isMultipleSelectPage( origin_field ) ){
                                ...
                                //botak fix price option Page2 
                                 if (origin_field.nbd_type == 'page2') {
                                    field.values = [field.values[field.values.length-1]];
                                 }
                                //end
        ...
        $scope.calculate_price = function(){
            ...
            var size_product_width = $scope.validate_float(<?php echo $product_size['product_width']; ?>); //Get width of product by size option
            var size_product_height = $scope.validate_float(<?php echo $product_size['product_height']; ?>); //Get height of product by size option
            var dimension_product_width = 0; //Get width of product by custom dimension option
            var dimension_product_height = 0; //Get height of product by custom dimension option
            var calculation_measure_base_on = 's'; //check calculation option is depended size or dimention option
            angular.forEach($scope.nbd_fields, function(field, field_id){
                if(field.enable){
                    var origin_field = $scope.get_field(field_id);
                    if (origin_field.nbd_type === 'size') {
                        
                        //Get product size by config OD or size printing option
                        if (origin_field.general.attributes.same_size === 'n') {
                            size_product_width = origin_field.general.attributes.options[field.value].product_width;
                            size_product_height = origin_field.general.attributes.options[field.value].product_height;
                        }
                        
                        //Calc price by size option
                        angular.forEach($scope.nbd_fields, function(tmp_field, tmp_field_id){
                            if(tmp_field.enable){
                                var tmp_origin_field = $scope.get_field(tmp_field_id);
                                if (tmp_origin_field.nbd_type === 'pricing_rates') {
                                    tmp_origin_field.general.attributes.options.map(option => {
                                        switch (option.calc_method) {
                                            case 'area':
                                                option.price[0] = ($scope.validate_float(size_product_width) * $scope.validate_float(size_product_height) * $scope.validate_float(option.rate)).toString();
                                                break;
                                            case 'perimeter':
                                                option.price[0] = (2 * ($scope.validate_float(size_product_width) + $scope.validate_float(size_product_height)) * $scope.validate_float(option.rate)).toString();
                                                break;
                                            case 'quantity':
                                                option.price[0] = ($scope.validate_float(option.quantity) * $scope.validate_float(option.rate)).toString();
                                                break;
                                        };
                                    });
                                }
                    ...
                    if (origin_field.nbd_type === 'dimension') {
                        //check if field dimension is enable so calculation option is depended dimention option
                        calculation_measure_base_on = 'd';
                        dimension_product_width = field.width;
                        dimension_product_height = field.height;
                    }
                }
            });
            ...
            angular.forEach($scope.nbd_fields, function(field, field_id){
                if (field.enable) {
                    var origin_field = $scope.get_field(field_id);
                    if (origin_field.nbd_type === 'calculation_option') {
                        if (calculation_measure_base_on === 's') {
                            origin_field = $scope.calculate_price_calculation_option(origin_field, size_product_width, size_product_height);
                        } else if (calculation_measure_base_on === 'd') {
                            origin_field = $scope.calculate_price_calculation_option(origin_field, dimension_product_width, dimension_product_height);
                        };
                    }
                }
            });
            ...
            angular.forEach($scope.nbd_fields, function(field, field_id){
                if (field.enable) {
                    var origin_field = $scope.get_field(field_id);
                                        var options_production_time = [];
                    if (origin_field.nbd_type === 'production_time' && origin_field.general.role_options) {
                        var time_work = 0;
                        var have_role_use = false;
                        var have_check_default = false;
                        angular.forEach(origin_field.general.role_options , function( role_options , key) {
                            if( $scope.user_role == role_options.role_name ) {
                                options_production_time_1 = role_options;
                                have_role_use = true;
                            }
                        ...
                        if(angular.isDefined(options_production_time)) {
                            options_pt = options_production_time.options[field.value]
                            var price_total_markup_percent = $scope.total_cart_price * parseInt(options_pt.markup_percent) / 100;
                            var price_discount_markup_percent = $scope.discount_by_qty * parseInt(options_pt.markup_percent) / 100;
                            if(price_total_markup_percent < options_pt.min_markup_percent) {
                                price_total_markup_percent = parseInt(options_pt.min_markup_percent);
                            }
                            if(price_discount_markup_percent < options_pt.min_markup_percent) {
                                price_discount_markup_percent = parseInt(options_pt.min_markup_percent);
                            }
                            $scope.discount_by_qty += price_discount_markup_percent;
                            $scope.total_cart_price += price_total_markup_percent;
                        }
                    }
                }
            });
            ...
            //CS botak calc price include tax
            $scope.total_cart_price_include_tax = $scope.convert_to_wc_price( (Math.round($scope.total_cart_price * 100) / 100 ) * (1 + <?php echo $tax_rate/100; ?>) );
            ...
            <?php if($change_base == 'yes' && !($options['display_type'] == 3 && count($options['bulk_fields']))): ?>
                ...
                var wrapEl = '#product-' + $scope.product_id + ' .summary';
                <?php endif; ?> 
            if('<?php echo $product->get_tax_status(); ?>' == 'none' ||  '<?php echo $product->get_tax_status(); ?>' == 'shipping') {
                var class_tax = '-regular';
            } else {
                var class_tax = '';
            }
            jQuery(wrapEl + ' .price'+class_tax+' .amount').html($scope.total_cart_price);
            jQuery(wrapEl + ' .nbo-base-price-html').html(nbds_frontend.total);
            jQuery('#product-' + $scope.product_id + ' .nbd-design-action-info .price'+class_tax+' .amount').html($scope.total_cart_price);
            jQuery('#product-' + $scope.product_id + ' .price-include-tax .amount').html($scope.total_cart_price_include_tax);
            jQuery('#product-' + $scope.product_id + ' .nbd-design-action-info .nbo-base-price-html').html(nbds_frontend.total);
            <?php endif; ?>
            if(jQuery('.merged_product_ajax').length > 0) {
                jQuery('.wrap-price-pro .price .amount').html($scope.total_cart_price);
                jQuery('.wrap-price-pro .price-regular .amount').html($scope.total_cart_price_include_tax);
            }
        ...
        $scope.calculate_price_base_measurement = function(origin_field, width, height){
            var nbd_dimention = "<?php echo nbdesigner_get_option('nbdesigner_dimensions_unit'); ?>",
                nbdesigner_dimensions_unit_rate = 1;
            var price_per_unit = 0, start_range = 0, end_range = 0, price_range = 0, addition_price = 0;
            
            //Convert nbdesigner_dimensions_unit to mm
            switch (nbd_dimention) {
                case "mm":
                    nbdesigner_dimensions_unit_rate = 1;
                    break;
                case "cm":
                    nbdesigner_dimensions_unit_rate = 10;
                    break;
                case "in":
                    nbdesigner_dimensions_unit_rate = 25.4;
                    break;
                case "ft":
                    nbdesigner_dimensions_unit_rate = 304.8;
                    break;
                default:
                    nbdesigner_dimensions_unit_rate = 1;
                    break;
            ...
            if (angular.isDefined(origin_field.general.multi_measure)) {
                for (let i = 0; i < origin_field.general.multi_measure.length; i++) { //Đh kiểu gì nhưng mà dùng each sẽ không nhận biến width, height bên trên
                    var measure = origin_field.general.multi_measure[i];
                    var measure_range = measure.measure_range;
                    //Convert measure_unit to sqmm
                    var measure_unit_rate = 1
                    ...
                    switch (measure.measure_unit) {
                        case "sqmm":
                            ...
                            break;
                        case "sqcm":
                            measure_unit_rate = 10;
                            break;
                        case "sqin":
                            measure_unit_rate = 25.4;
                            break;
                        case "sqft":
                            measure_unit_rate = 304.8;
                            break;
                        default:
                            measure_unit_rate = 1;
                            break;
                    ...
                    var rate = nbdesigner_dimensions_unit_rate / measure_unit_rate; //Convert custom unit to OD unit

                    var calc_value = 0, price = 0;
                    switch (measure.caculation_method) {
                        case 'area':
                            calc_value = $scope.validate_float(width) * rate * $scope.validate_float(height) * rate ; // (width * height)
                            break;
                        case 'perimeter':
                            calc_value = 2 * ($scope.validate_float(width) * rate + $scope.validate_float(height) * rate) ; // 2 * (width + height)
                            break;
                        case 'top-bottom':
                            calc_value = 2 * $scope.validate_float(width) * rate ; // 2 * width
                            break;
                        case 'left-right':
                            calc_value = 2 * $scope.validate_float(height) * rate ; // 2 * height
                            break;
                        case 'custom-formula':
                            var _width = $scope.validate_float(width) * rate;
                            var _height = $scope.validate_float(height) * rate;
                            var custom_formular = measure.custom_formular;
                            custom_formular = custom_formular.replace(/{width}/g, _width);
                            custom_formular = custom_formular.replace(/{height}/g, _height);
                            //custom_formular = custom_formular.replace(/{rate}/g, price_per_unit);

                            // sanitize input
                            custom_formular = custom_formular.replace(/[^a-z0-9+\-.*\/()%]/g,"");
                            // convert alphabet to $variabel 
                            custom_formular = custom_formular.replace(/([a-z])+/i, "\$0"); 
                            // convert percentages to decimal
                            custom_formular = custom_formular.replace(/([+-])([0-9]{1})(%)/g, "*(1\$1.0\$2)");
                            custom_formular = custom_formular.replace(/([+-])([0-9]+)(%)/g, "*(1\$1.\$2)");
                            custom_formular = custom_formular.replace(/([0-9]{1})(%)/g, ".0\$1");
                            custom_formular = custom_formular.replace(/([0-9]+)(%)/g, ".\$1");

                            if ( custom_formular != "" ){
                                calc_value = eval( custom_formular );
                            }
                            if (calc_value == null) {
                                calc_value = 0;
                    ...
                    if( angular.isDefined( measure.measure_type ) && measure.measure_type == 'u' ) {
                        price_per_unit = $scope.validate_float(measure_range[0][2]);
                    } else if ( angular.isDefined( measure.measure_type ) && measure.measure_type == 'r' ) {
                        angular.forEach(measure_range, function(range, key){
                            start_range = $scope.validate_float(range[0]);
                            end_range = $scope.validate_float(range[1]);
                            price_range = $scope.validate_float(range[2]);
                            if( start_range <= calc_value && ( calc_value <= end_range || end_range == 0 ) ){
                                price_per_unit = price_range;
                            }
                            if( start_range <= calc_value && key == ( measure_range.length - 1 ) && calc_value > end_range  ){
                                price_per_unit = price_range;
                            }
                        });
                    }

                    price = price_per_unit * calc_value;
                    if (price < measure.minimum_price) {
                        price = measure.minimum_price;
                    }
                }
                addition_price += $scope.validate_float(price);
            }
         ...
         */
        $scope.calculate_price_calculation_option = function(origin_field, width, height){
            var nbd_dimention = "<?php echo nbdesigner_get_option('nbdesigner_dimensions_unit'); ?>",
                nbdesigner_dimensions_unit_rate = 1;
            ...
            //CS botak dimention break
            if (angular.isDefined(origin_field.general.attributes.options)) {
                for (let i = 0; i < origin_field.general.attributes.options.length; i++) { //Đh kiểu gì nhưng mà dùng each sẽ không nhận biến width, height bên trên
                    var option = origin_field.general.attributes.options[i];
                    var measure_range = option.measure_range;
                    //Convert measure_unit to sqmm
                    var measure_unit_rate = 1
                    ...
                    switch (origin_field.general.caculation_method) {
                        ...
                        case 'custom-formula':
                            var _width = $scope.validate_float(width) * rate;
                            var _height = $scope.validate_float(height) * rate;
                            var custom_formular = origin_field.general.custom_formular;
                            custom_formular = custom_formular.replace(/{width}/g, _width);
                            custom_formular = custom_formular.replace(/{height}/g, _height);
                            //custom_formular = custom_formular.replace(/{rate}/g, price_per_unit);
                            ...
                            // convert alphabet to $variabel 
                            // custom_formular = custom_formular.replace(/([a-z])+/i, "\$0"); 
                            custom_formular = custom_formular.replace(/([a-z])+/i, "\$1");  // 24/7 fix lỗi đổi options
                            // convert percentages to decimal
                            custom_formular = custom_formular.replace(/([+-])([0-9]{1})(%)/g, "*(1\$1.0\$2)");
                            custom_formular = custom_formular.replace(/([+-])([0-9]+)(%)/g, "*(1\$1.\$2)");
                            custom_formular = custom_formular.replace(/([0-9]{1})(%)/g, ".0\$1");
                    ...
                    if( angular.isDefined( origin_field.general.measure_type ) && origin_field.general.measure_type == 'u' ) {
                        price_per_unit = $scope.validate_float(measure_range[0][2]);
                    } else if ( angular.isDefined( origin_field.general.measure_type ) && origin_field.general.measure_type == 'r' ) {
                        angular.forEach(measure_range, function(range, key){
                            start_range = $scope.validate_float(range[0]);
                            end_range = $scope.validate_float(range[1]);
                            price_range = $scope.validate_float(range[2]);
                    ...
                    price = price_per_unit * calc_value;
                    if (price < origin_field.general.minimum_price) {
                        price = origin_field.general.minimum_price;
                    }

                    if(origin_field.nbd_type == 'calculation_option') {
                        if(origin_field.general.measure == 'y') {
                            option.price[0] = $scope.validate_float(price).toString();
                        }
                    } 
                }
            }
