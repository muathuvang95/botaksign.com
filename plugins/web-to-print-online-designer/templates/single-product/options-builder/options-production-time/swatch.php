                ...
                <input ng-change="check_valid();updateMapOptions('<?php echo $field['id']; ?>')" value="<?php echo $key; ?>" ng-model="nbd_fields['<?php echo $field['id']; ?>'].value" name="nbd-field[<?php echo $field['id']; ?>][value]" 
                    ...
                    <?php 
                        if( isset($form_values[$field['id']]) ){
                            ...
                            checked( isset($attr['check_default']) ? $attr['check_default'] : 'off', 'on' ); 
                        }
                    ?> />
                <label class="nbd-swatch" style="<?php echo 'background: url('.$attr['image_url'] . ') 0% 0% / cover'; ?>"
                    title="<?php echo $attr['name']; ?>" for='nbd-field-<?php echo $field['id'].'-'.$key; ?>'>
                    <span class="nbd-swatch-tooltip"><?php echo $attr['name']; ?> - <span><b>{{time_work[<?php echo $key; ?>]}} Hours</b></span></span>
                </label>
                <?php endforeach; ?>
            </div>
        </div>
