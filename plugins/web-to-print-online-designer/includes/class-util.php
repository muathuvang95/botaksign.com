...
class NBD_Image {
    ...
    public static function pdf2imageConverPng($path, $dpi = 300, $type = 'png'){
        ...
        for( $i = 0; $i < $number_pages; $i++ ){
            ...
            //$im->stripImage();
            $im->writeImage(str_replace(".pdf","." . $type, $path));
            $im->clear();
            $im->destroy();
        }
        $document->clear();
...
function default_frontend_setting(){
    return apply_filters( 'nbdesigner_default_frontend_settings', array(
        ...
        'nbdesigner_pexels_api_key'                 => '563492ad6f9170000100000147b95f140fe441b858072ac5940c9ba0',
...
//    }
    ...
    if(!get_post_meta($product_id, '_designer_upload_cs', true)) {
        ...
        $data['upload']['disallow_type'] = nbdesigner_get_option('nbdesigner_disallow_upload_file_type', '');
        $data['upload']['number'] = nbdesigner_get_option('nbdesigner_number_file_upload', '');
        $data['upload']['minsize'] = nbdesigner_get_option('nbdesigner_minsize_upload_file', '');
        $data['upload']['maxsize'] = nbdesigner_get_option('nbdesigner_maxsize_upload_file', '');
        $data['upload']['mindpi'] = nbdesigner_get_option('nbdesigner_mindpi_upload_file', '');
    }
    //
...
function nbd_zip_files_and_download($file_names, $archive_file_name, $nameZip, $option_name = array(), $download = true, $upload_aws = false){
...
//                $name = $path_arr[count($path_arr) - 2] . '_' . basename($file);
                //CS botak name of design file
                $name = basename($file);
            }
            if(strpos($file , 's3.amazonaws.com') > 0 ) {
                ...
                $tmpfile = '';
                while (!@feof($handle)) {
                    $tmpfile .= @fread($handle, $chunksize);
                ...
                try {
                    $zip->addFromString($name, $tmpfile);
                } catch( Exception $e ){
                    echo 'File too big' . $e->getMessage();
                } 
            } else {
    ...
    if($upload_aws){
        ...
        if ($s3->putObjectFile($archive_file_name, "$_bucket", $nameZip, S3::ACL_PUBLIC_READ)) {
            $contents = $s3->getBucket($_bucket);
            foreach ($contents as $file){
                if($file['name'] == $nameZip){
                    $fname = $file['name'];
                    $furl = "https://$_bucket.s3.".$amazonRegion.".amazonaws.com/".$fname;
                } else {
                    $furl = "https://$_bucket.s3.".$amazonRegion.".amazonaws.com/".$nameZip;
                }
                return $furl;
            }
        }else{
    ...
    if( nbdesigner_get_option( 'nbdesigner_enable_cloud2print_api', 'no' ) == 'yes' ){
        ...
        foreach( $pdfs as $key => $_pdf ){
                    ...
                    if( count( $resource_pdfs ) && $config->pdfStacks[$key] != '' ){
                        ...
                        foreach( $stack as $pos ){
                            if( $pos == 'P' ){
                                ...
                                // fix link pdf s3
                                $_bucket = get_option('nbdesigner_aws_bucket', false);
                                $pdf_path_s3 = 'https://'.$_bucket.'.s3.ap-southeast-1.amazonaws.com/design-templates' . $resource_pdf['origin_pdf']; //update link pdf s3
                                if( botak_check_link_exists_s3($pdf_path_s3) ) {
                                    $fileContentS3 = file_get_contents($pdf_path_s3,'rb');
                                    $pdf_path = StreamReader::createByString($fileContentS3);
                                } else {
            ...
            if( $datas[$key]['show_overlay'] == 1 && $datas[$key]['include_overlay'] == 1 && $layout != 'c' ){
                ...
                $filetype = explode('.', basename($overlay));
                $file_exten = $filetype[count($filetype) - 1];
                $check_over_img = Nbdesigner_IO::checkFileType(basename($overlay), array('jpg','jpeg','png','svg'));
                if( $check_over_img ){
                    //$pdf->Image($overlay, $mLeft + $cdLeft, $mTop + $cdTop, $cdWidth,$cdHeight, '', '', '', false, '');
                    if ($file_exten == 'svg') {
                        //$pdf->ImageSVG($overlay, round( $datas[$key]['overlay_real_left'] ), round( $datas[$key]['overlay_real_top'] ),  $cdWidth, $cdHeight, '', '', '',0, false);
                        // custom botak fix background svg
                        $image = new Imagick();
                        $image->setBackgroundColor(new ImagickPixel('transparent'));
                        $image->readImage($overlay);
                        //$image->resizeImage( $cdWidth, $cdHeight, imagick::FILTER_LANCZOS, 1, true);
                        $image->setImageFormat('png32');
                        $pdf->Image('@'.$image, round( $datas[$key]['overlay_real_left'] ), round( $datas[$key]['overlay_real_top'] ),  $cdWidth, $cdHeight, '', '', '',0, false);
                    } else {
                       $pdf->Image($overlay, round( $datas[$key]['overlay_real_left'] ), round( $datas[$key]['overlay_real_top'] ),  round( $datas[$key]['overlay_real_width'] ), round( $datas[$key]['overlay_real_height'] ), '', '', '', false); //CS botak overlay size
                    }  
                }
            }
            if( $watermark ){
...
function custom_nbd_export_pdfs( $nbd_item_key, $watermark = false, $force = false, $showBleed = 'no', $extra = null ){
    if( !class_exists('TCPDF') ){
        require_once( NBDESIGNER_PLUGIN_DIR.'includes/tcpdf/tcpdf.php' );
    }
    require_once( NBDESIGNER_PLUGIN_DIR . 'includes/fpdi/autoload.php' );
            ...
            if( $customer_design != '' ){
                ...
                if( isset( $config->originPDFs ) && isset( $config->originPDFs[$key] ) && isset( $config->pdfStacks ) ){
                    ...
                    if( count( $resource_pdfs ) && $config->pdfStacks[$key] != '' ){
                        ...
                        $stack          = explode( '_', $config->pdfStacks[$key] );
                        $pdf_index      = 0;
                        if($pdfFile == $resource_pdfs['origin_pdf'] ) {
                            $include_pdf = false;
                        }
                        foreach( $stack as $pos ){
                            if( $pos == 'P' ){
...
function nbd_convert_svg_url($path, $file){
    ...
    foreach ($cpEls as $cpEl) {
        ...
        $cpMtArr    = nbd_get_transfrom_arr( $cpTm, 'matrix' );
        $cpTrArr    = nbd_get_transfrom_arr( $cpTm, 'translate' );
        $sx         = 1 / $cpMtArr[0];
        $sy         = 1 / $cpMtArr[3];
...
function nbd_get_transfrom_arr( $attribute, $type ){
    $transform = array();
        ...
        foreach($countries as $con){
            if(strtolower($con) == strtolower($country_code) ){
                ...
                $users[] = array(
                    ...
                    'c_title'       => (isset( $contact_acf_fields['user_title'] ) && $contact_acf_fields['user_title']['value'] != '') ? $contact_acf_fields['user_title']['value'] : esc_html__('Title', 'web-to-print-online-designer'),
...
function nbd_get_user_contact_sheet( $id = false ){
    ...
    $infos          = array(
        ...
        'title'     =>  isset( $acf_fields['user_title'] ) ? $acf_fields['user_title']['value'] : esc_html__('Title', 'web-to-print-online-designer'),
        'mobile'    =>  isset( $acf_fields['user_mobile'] ) ? $acf_fields['user_mobile']['value'] : esc_html__('0123456789', 'web-to-print-online-designer'),
        ...
        if( isset( $acf_fields[$con . '_contact'] ) && count($acf_fields[$con . '_contact']['value']) ){
            foreach ($acf_fields[$con . '_contact']['value'] as $key => $cid){
                ...
                $infos['contact'][$con][$key] = array(
                    'c_full_name'   => $contact_infos['full_name']['value'],
                    'c_title'       => (isset( $contact_acf_fields['user_title'] ) && $contact_acf_fields['user_title']['value'] != '') ? $contact_acf_fields['user_title']['value'] : esc_html__('Title', 'web-to-print-online-designer'),
                    'c_mobile'      => isset( $contact_acf_fields['user_mobile'] ) ? 'Mob ' . $contact_acf_fields['user_mobile']['value'] : 'Mob ' . esc_html__('0123456789', 'web-to-print-online-designer'),
                    'c_email'       => $contact_infos['email']['value'],
                    'c_avatar'      => nbd_convert_tif_to_png($contact_infos['email']['value'] . '.tif', 'small')
                );
            }
...
function nbd_get_redirect_url(){
    ...
    switch( $rd ){
        ...
        case 'print_option':
            ...
            break;
        case 'reupload':                                            // Botak custom reupload Phase 3
            $order_id           = absint( $_GET['order_id'] );
            $redirect_url       =  wc_get_page_permalink( 'myaccount' ).'orders/?order_again='.$order_id.'&edit_order='.$order_id;
            break;
        default:
            $redirect_url       = $rd;
            break;
...
function nbd_get_items_product_grouped($product_id) {
    $data       = array();
        ...
        if ( is_array( $items ) && count( $items ) > 0 ) {
            foreach ( $items as $item ) {
                ...
                $data[]    = array(
                    ...
                    'qty' => (float) ( isset( $item_data[1] ) ? $item_data[1] : 0 )
                );
            }
        }
    }
