...
class My_Design_Endpoint {
    ...
    public function nbd_gallery_func($atts, $content = null) {
        ...
        $atts = shortcode_atts(array(
            ...
            'total'                 => $pid != 0 ? $this->count_total_template($pid, false, $cat) : $this->count_total_template(false, false, $cat)
        ), $atts);
        if( $atts['per_row'] > 6 ) $atts['per_row'] = 6;
        $atts['templates']  = $this->nbdesigner_get_templates_by_page($page, absint($atts['row']), absint($atts['per_row']), $pid, false, false, $cat, $tag, $color, '', $search, $search_type);
        $atts['total']      = $this->nbdesigner_get_templates_by_page($page, absint($atts['row']), absint($atts['per_row']), $pid, true, false, $cat, $tag, $color, '', $search, $search_type, true);
        ob_start();
    ...
    public function nbd_get_next_gallery_page(){
        ...
        $templates = $this->nbdesigner_get_templates_by_page($page, $row, $per_row, $pid, false, false, $cat, $tag, $color, '', $search, $search_type);
        $favourite_templates = $this->get_favourite_templates();
        ob_start();
        if( count($templates) ){
            $result['flag'] = 1;
    ...
    public static function nbdesigner_get_templates_by_page($page = 1, $row = 5, $per_row = 3, $pid = false, $get_all = false, $user_id = false, $cat = false, $tag = '', $color = '', $type = '', $search = '', $search_type = '', $return_total = false ){
        ...
        if( $pid ){
            ...
            if(is_array($products) && count($products) ){
                ...
                // fix lỗi khi lấy total nhưng chạy vào trường hợp này sẽ gây lỗi site
                if(!$return_total) {
                    return array();
                } 
            }
