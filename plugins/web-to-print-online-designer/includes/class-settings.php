...
if( !class_exists('Nbdesigner_Settings') ) {
	...
	public function get_option_type($key) {
        ...
        public static function output_item( $parameters, $product_id = null ) {
            ...
            //text,number, checkbox, password
            //CS botak download design
            if ($type == 'text' || $type == 'password' || $type == 'number' || $type == 'checkbox' || $type == 'colorpicker' || $type == 'upload') {
                $additional_attrs = '';
                $relation_attr = '';
            ...
            if($show):
            ?>
            <tr data-option-id="<?php esc_attr_e( $id ); ?>" data-depend="<?php esc_attr_e( $depend_on['id'] ); ?>" 
                data-depend-value="<?php esc_attr_e( $depend_on['value'] ); ?>" data-depend-operator="<?php esc_attr_e( $depend_on['operator'] ); ?>">
                <th scope="row" <?php esc_attr_e( $type === 'section_title' ? 'colspan="2" class="nbdesigner-section-title"' : '' ); ?>>
