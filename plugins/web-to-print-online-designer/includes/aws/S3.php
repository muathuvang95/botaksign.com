...
class S3 {
	...
	*/
	...
	public static function listObject($bucket , $uri) {
		$rest = new S3Request('GET', $bucket, '');
		$response = $rest->getResponse();
		if ($response->error === false && $response->code !== 200)
			$response->error = array('code' => $response->code, 'message' => 'Unexpected HTTP status');
		if ($response->error !== false) {
			trigger_error(sprintf("S3::listObject(): [%s] %s", $response->error['code'], $response->error['message']), E_USER_WARNING);
			return false;
		}
		$results = array();
		if (isset($response->body, $response->body->Contents)) {
			foreach ($response->body->Contents as $c) {
				 $name_array = explode('/' , $c->Key);
				...
				if( $uri != '' && $path == $uri) {
					$link_file = 'https://'.$bucket.'.s3.amazonaws.com/'.$c->Key;
					$ext = pathinfo($link_file, PATHINFO_EXTENSION);
					if($ext) {
						$results[] = $link_file;
					}
				}
