...
class Nbdesigner_Plugin {
    ...
    public function init(){
        ...
        add_action( 'woocommerce_checkout_order_processed', array($this, 'nbdesigner_detail_order_custom_aws') );
    ...
    public function ajax(){
        ...
        $ajax_events = array(
            ...
            'nbd_check_before_upload_design_file'       => true,  // custom upload js S3
            ...
            'nbd_get_user_id'                           => true,
            'nbd_crop_image'                            => true,
            'nbd_fix_pdf_font'                          => true,
            'nbd_save_and_download_customer_design'     => true,
    ...
    public function nbd_get_user_id(){
        wp_send_json(
            array(
    ...
    public function woocommerce_hook(){
        ...
        add_filter( 'btk_cart_item_thumbnail', array( $this, 'render_design' ), 1, 3 );
    ...
    public function nbdesigner_admin_enqueue_scripts( $hook ){
        ...
        if ( in_array( $hook, apply_filters( 'nbd_admin_hooks_need_asset', array( 'post.php', 'post-new.php', 'nbdesigner_page_nbdesigner_manager_fonts', 'nbdesigner_page_nbdesigner_manager_arts', 'toplevel_page_nbdesigner', 'nbdesigner_page_nbdesigner_manager_product', 'toplevel_page_nbdesigner_shoper', 'nbdesigner_page_nbdesigner_frontend_translate', 'nbdesigner_page_nbdesigner_tools', 'nbdesigner_page_manage_color' ) ) ) ){
            ...
            wp_localize_script( 'admin_nbdesigner', 'admin_nbds', array(
                ...
                'nbd_unit'      => nbdesigner_get_option('nbdesigner_dimensions_unit')
            ));                
    ...
    public function frontend_enqueue_scripts(){
        ...
        $depends = apply_filters( 'nbd_depend_js', array( 'jquery', 'jquery-blockui', 'wp-util' ) );
        $nbdesigner_enable  = get_post_meta( get_the_ID(), '_nbdesigner_enable', true );
        wp_register_script( 'nbdesigner', NBDESIGNER_JS_URL . 'nbdesigner.js?t='.strtotime("now"), $depends, NBDESIGNER_VERSION );
        $args = apply_filters( 'nbd_js_object', array(
            'url'                   => admin_url( 'admin-ajax.php' ),
            'sid'                   => session_id(),
            'check_ne'              => $nbdesigner_enable,
            'nonce'                 => wp_create_nonce('save-design'),
            'nonce_get'             => wp_create_nonce('nbdesigner-get-data'),
...
//            );            
    ...
    */    
        ...
        if(isset($_GET['order_id'])){
            ...
            if(isset($_GET['download-all']) && ($_GET['download-all'] == 'true')){
                ...
                foreach($products AS $order_item_id => $product){
                    if(wc_get_order_item_meta($order_item_id, '_nbd') || wc_get_order_item_meta($order_item_id, '_nbu')){
                        ...
                        if( $nbd_item_key ){
                            $pdf_path   = NBDESIGNER_CUSTOMER_DIR .'/'. $nbd_item_key . '/customer-pdfs';
                            $list_pdf   = Nbdesigner_IO::get_list_files_by_type($pdf_path, 1, 'pdf');
                            // Empty the directory before creating the file
                            if(count($list_pdf) > 0){
                                foreach($list_pdf as $key => $file){
                                   unlink($file);
                                }
                            }
                            nbd_export_pdfs( $nbd_item_key, false, false, 'no' ); // Create file pdf
                            $list_pdf   = Nbdesigner_IO::get_list_files_by_type($pdf_path, 1, 'pdf');
                            if(count($list_pdf) > 0){
                                foreach($list_pdf as $key => $file){
                                    $file_ext = pathinfo($file, PATHINFO_EXTENSION);
                                    $new_file_name = $pdf_path . '/' .$order_id . '_' . $key . '_' . $product->get_name() . '.' . $file_ext ;
                                    $check = rename($file, $new_file_name);
                                    if ($check) {
                                        $zip_files[] = $new_file_name;
                                    } else {
                                        $zip_files[] = $file;
                                    }
                                }
                ...
                if( !count($zip_files) ){
                    ...
                    if($this->checlLinkDownload($order_id)){
                        $link_aws = nbd_zip_files_and_download( $zip_files, $pathZip, $nameZip, $option_name = array(), $download = false, $upload_aws = true );
                        $data_insert = array(
                            'id_order'      => (int)$order_id,
                            'link_download'             => $link_aws,
                        );
                        $wpdb->insert("{$wpdb->prefix}aws_order_download", $data_insert );
                        
                        if($link_aws) {
                            unlink($pathZip); // botak remove file download when upload file to s3
                            NBD_Download::download_file($link_aws, $nameZip);
                        } 
    ...
    public function setting_design() {
        ...
        $setting_upload_cs = get_post_meta($post_id, '_nbdesigner_enable_upload_cs', true); 
        $enable_design_help = get_post_meta($post_id, '_nbdesigner_enable_design_help', true); //CS botak
...
//                }  
    ...
    public function save_settings($post_id) {
        ...
        $setting_upload_cs = $post['_designer_upload_cs'];
        update_post_meta($post_id, '_designer_upload_cs', $setting_upload_cs);
        $enable_design_help = $post['_nbdesigner_design_help']; //CS botak
        $option = serialize($post['_nbdesigner_option']);
        $setting_design = serialize($post['_designer_setting']);
        if($setting_upload_cs) {
            $setting_upload = serialize($post['_designer_upload']);
        } 
        else {
            $setting_upload_df['allow_type'] = nbdesigner_get_option('nbdesigner_allow_upload_file_type', '');
            $setting_upload_df['disallow_type'] = nbdesigner_get_option('nbdesigner_disallow_upload_file_type', '');
            $setting_upload_df['number'] = nbdesigner_get_option('nbdesigner_number_file_upload', '');
            $setting_upload_df['minsize'] = nbdesigner_get_option('nbdesigner_minsize_upload_file', '');
            $setting_upload_df['maxsize'] = nbdesigner_get_option('nbdesigner_maxsize_upload_file', '');
            $setting_upload_df['mindpi'] = nbdesigner_get_option('nbdesigner_mindpi_upload_file', '');
            $setting_upload = serialize($setting_upload_df);
        }
        ...
        update_post_meta($post_id, '_nbdesigner_enable_upload_cs', $setting_upload_cs);
        update_post_meta($post_id, '_nbdesigner_enable_design_help', $enable_design_help); //CS botak
    }
    ...
    public function nbdesigner_button() {
        ...
        if(isset($_POST['action']) && $_POST['action'] != 'get_content_product') return ''; /*Hidden button Start Design on third-party plugin as Quick view*/
        ...
        foreach ($product_config as $key => $val){
            ...
            if(file_exists($p_img)){
                ...
                if( $val["show_overlay"] == '1' ){
                    ...
                    if($over_ext == "png"){
                        $image_overlay = NBD_Image::nbdesigner_resize_imagepng($path_img_overlay, $val['overlay_width'] * $scale, $val['overlay_height'] * $scale); //CS botak overlay size
                    }else if($over_ext == "jpg" || $over_ext == "jpeg"){
                        $image_overlay = NBD_Image::nbdesigner_resize_imagejpg($path_img_overlay, $val['overlay_width'] * $scale, $val['overlay_height'] * $scale); //CS botak overlay size
                    }else{
                        $val["show_overlay"] = '0';
                    }
                }
                ...
                if( $val["show_overlay"] == '1' && $image_overlay ){
                    ...
                    imagecopy($image, $image_overlay, ($val['overlay_left'] - $val["img_src_left"]) * $scale, ($val['overlay_top'] - $val["img_src_top"]) * $scale, 0, 0, (int) $val['overlay_width'] * $scale, (int) $val['overlay_height'] * $scale);
                }
                if($design_format == 'png'){
...
//                }
    ...
    public function nbd_save_customer_design(){
        ...
        $reupload   = ( isset( $_POST['reupload'] ) && $_POST['reupload'] != '' ) ? wc_clean( $_POST['reupload'] ) : '';
        $item_id    = ( isset( $_POST['item_id'] ) && $_POST['item_id'] != '' ) ? wc_clean( $_POST['item_id'] ) : '';
        if( $reupload == 'reupload' && $task == 'new') {
            if( $item_id ) {
                $nbd_item_key = substr(md5(uniqid()),0,5).rand(1,100).time();
                wc_update_order_item_meta($item_id, '_nbd' , $nbd_item_key  );
            }
        }
        if( $item_id ) {
            wc_update_order_item_meta($item_id , '_nbd_check', 1);
        }
        // END
        ...
        if( false != $save_status ){
            ...
            if( $task == 'new' && $reupload != 'reupload'){      // BOTAK custom reupload phase 3
                if( $task2 == 'update' ){
                    WC()->session->set($cart_item_key. '_nbd', $nbd_item_key);
                    if( !isset(WC()->cart->cart_contents[ $cart_item_key ]['nbd_item_meta_ds']) ) WC()->cart->cart_contents[ $cart_item_key ]['nbd_item_meta_ds'] = array();
                    WC()->cart->cart_contents[ $cart_item_key ]['nbd_item_meta_ds']['nbd'] = $nbd_item_key;
            ...
            if( $task == 'create' ){
                if( $design_type == 'art' ){
                    ...
                    if(!can_edit_nbd_template()){
                        $result['mes'] = esc_html__('You have not permission to create or edit template', 'web-to-print-online-designer'); echo json_encode($result); wp_die();
                    }else{
        ...
        //CS botak save images paid
        if (isset($_POST['images_paid']) && is_array($_POST['images_paid'])) {
            WC()->session->set('nbd_images_paid_'.$nbd_item_cart_key, serialize($_POST['images_paid']));
        }
    ...
    public function render_design($thumbnail = '', $cart_item = null, $cart_item_key = null) {
        if ($cart_item_key && ( is_cart() )) {
            ...
            if ( $is_nbdesign && $_show_design == 'yes' ) {
                ...
                if( isset($nbd_session) ){
                    ...
                    $html          .= '<div id="'.$id.'" class="nbd-custom-design nbd-cart-item-design">';
                    ...
                    // $html          .= '<p>'. esc_html__('Upload via Editor', 'web-to-print-online-designer') .$remove_design.'</p>';
                    ...
                    foreach ($list as $img) {
                        ...
                        if($is_first_design) {
                            $first_design = '<img class="nbd_cart_item_design_preview btk-first-design" src="' . $src . '"/>';
                            $is_first_design = false;
                        } else {
                ...
                if( isset( $nbu_session ) ){
                    ...
                    $remove_upload  = is_cart() ? '<a class="remove nbd-cart-item-remove-file" href="javascript:void(0)" onclick="NBDESIGNERPRODUCT.remove_design(\'upload\', \''.$cart_item_key.'\')">&times;</a>' : '';
                    ...
                    $files          = botak_get_list_file_s3('reupload-design/'. $nbu_session); // BOTAK S3
                    ...
                    foreach ($files as $file) {
                        ...
                        if($is_first_design) {
                            $first_design = '<div class="nbd-cart-item-upload-preview-wrap"><a target="_blank" href='.$file_url.'><img class="nbd-cart-item-upload-preview" src="' . $src . '"/></a><p class="nbd-cart-item-upload-preview-title">'. basename($file).'</p></div>';
                            $is_first_design = false;
                        } else {
...
//            }
    ...
    public function nbd_check_before_upload_design_file(){
        ...
        $new_name = time(). '_' .sanitize_file_name($_POST["file_name_s3"]);
        ...
        if( $reupload == 'reupload' && $task == 'new') {
            if( $item_id ) {
                ...
                wc_update_order_item_meta($item_id, '_nbu' , $nbu_item_key  );
                wc_update_order_item_meta($item_id , '_nbu_check', 1);
            }
        }
    ...
    public function nbd_upload_design_file(){
        $file_s3 = json_decode(stripslashes($_POST['file_s3']) , true);
        if ((!wp_verify_nonce($_POST['nonce'], 'save-design') && NBDESIGNER_ENABLE_NONCE) || (!isset($_FILES) && !isset($file_s3) ) ) {
            die('Security error');
        } 
        $result = array(
            'flag'  =>  0,
        ...
        if(get_post_meta($product_id, '_designer_upload_cs', true)  ) {
            ...
            $setting_upload_df['maxsize'] = nbdesigner_get_option('nbdesigner_maxsize_upload_file', '');
            $setting_upload_df['mindpi'] = nbdesigner_get_option('nbdesigner_mindpi_upload_file', '');
            $upload_setting = $setting_upload_df;
        }
        ...
        $ext                = strtolower( $this->nbdesigner_get_extension( $file["name"] ) );
        ...
        if( $minsize > $file["size"] || ($max_size != 0 && $max_size < $file["size"]) ){
            $checkSize = true;
        }
        if( $checkSize || $checkExt || $checkDPI ){
            ...
            // custom botak s3
            $upload_origin = '';
            $upload_origin = $file["tmp_name"];
            if($upload_load_s3) {
                $new_name = isset($_POST['file_name']) ? $_POST['file_name'] : basename($upload_origin);
            } else {
                $new_name = time(). '_' .sanitize_file_name($file["name"]);
                $new_name = nbd_alias($new_name);
                $upload_origin = nbd_upload_file_custom_to_s3($nbu_item_key.'/'.$new_name , $file["tmp_name"] , 'reupload-design' );
            }
            ...
            if($upload_origin){
                ...
                if( nbdesigner_get_option('nbdesigner_create_preview_image_file_upload') == 'yes' ){                    
                    $jpg_preview = botak_create_preview_img_upload_s3( $nbu_item_key.'_preview/'.$new_name , $upload_origin , 'reupload-design'); 
                    if( $jpg_preview ){
                        $preview_file = $jpg_preview;
                    }else{
    ...
    //         }
    //         //custom botak s3
    //         $upload_origin = nbd_upload_file_custom_to_s3($nbu_item_key.'/'.$new_name , $_FILES['file']["tmp_name"] , 'reupload-design' ); 
    //         if($upload_origin){
    //             $result['mes'] = esc_html__('Upload success !', 'web-to-print-online-designer');  
    ...
    public function nbdesigner_customer_upload(){
        ...
        $allow_extension    = array( 'jpg', 'jpeg', 'png', 'gif', 'svg', 'pdf' );
        ...
        if( isset($s3_preview) && $s3_preview ) {
            unlink($path_preview);
        }
		/*
        $_pdfString = strpos($res['src'], 'pdf');

        if( $_pdfString !== false ){

            $dpi    = 300;

            $res['DIR'] = NBDESIGNER_TEMP_DIR . $path['date_path'] . '/' . $new_name  ;

            NBD_Image::pdf2imageConverPng(NBDESIGNER_TEMP_DIR . $path['date_path'] , $dpi, 'jpg');

        }

        $res['src'] = str_replace(".pdf",".jpg", NBDESIGNER_TEMP_URL . $path['date_path']);
		*/
    ...
    public function update_files_upload( $files, $nbu_session = '' ){
        $files          = explode('|', $files);
        $path           = 'reupload-design/' . $nbu_session;
        // $path           = NBDESIGNER_UPLOAD_DIR . '/' . $nbu_session;
        // $list_files     = Nbdesigner_IO::get_list_files($path);

        // custom botak S3 : delete file s3
        $list_files     = botak_get_list_file_s3($path);
        foreach ($list_files as $file){
            $filename = basename( $file );
            if( !in_array( $filename, $files ) && !strpos($file , '.pdf.jpg')){
                // unlink($path. '/' .$filename);
                //botak_remove_obj_from_s3($path. '/' .$filename);
            }
        }
    }
    public function nbd_update_customer_upload(){
        ...
        $reupload   = ( isset( $_POST['reupload'] ) && $_POST['reupload'] != '' ) ? wc_clean( $_POST['reupload'] ) : '';
        $item_id    = ( isset( $_POST['item_id'] ) && $_POST['item_id'] != '' ) ? wc_clean( $_POST['item_id'] ) : '';
        $nbu_item_key    = ( isset( $_POST['nbu_item_key'] ) && $_POST['nbu_item_key'] != '' ) ? wc_clean( $_POST['nbu_item_key'] ) : '';

        if( $design_type == '' && !nbd_check_cart_item_exist($cart_item_key) && $reupload != 'reupload' ) {  // BOTAK custom reupload Phase 3
...
//                    $pdf->Line($mLeft + $bgWidth - $bRight, $mTop + $bgHeight, $mLeft + $bgWidth - $bRight, $mTop + $bgHeight + $mBottom, array('color' => array(0,0,0), 'width' => 0.05));
        ...
        if( $nbd_item_key || $nbu_item_key ){
            ...
            if(( isset($item["item_meta"]["_nbd"]) || isset($item["item_meta"]["_nbu"]) ) && $show_design_in_order == 'yes'){
                ...
                if( isset($item["item_meta"]["_nbu"]) ){
                    ...
                    $files              = botak_get_list_file_s3('reupload-design/'. $nbu_item_key); // Custom BOTAK S3
                    if(!$files) {
                        $files              = Nbdesigner_IO::get_list_files( NBDESIGNER_UPLOAD_DIR .'/'. $nbu_item_key );
                    }
    ...
    public function nbd_save_and_download_customer_design() {
        if ((!wp_verify_nonce($_POST['nonce'], 'save-design') && NBDESIGNER_ENABLE_NONCE) || !isset($_POST['product_id'])) {
            die('Security error');
        }
        $result = array(
        ...
        try {
            $zip_files  = array();
            $files      = array();
            if(isset($_POST['download-type']) && ($_POST['download-type'] == 'png-hires' || $_POST['download-type'] == 'jpg-hires' || $_POST['download-type'] == 'pdf' || $_POST['download-type'] == 'png' || $_POST['download-type'] == 'jpg' || $_POST['download-type'] == 'cmyk' || $_POST['download-type'] == 'svg' || $_POST['download-type'] == 'ibg')){
                $type  = sanitize_text_field( $_POST['download-type'] );
                if ($nbd_item_key != null) {
                    $path = NBDESIGNER_CUSTOMER_DIR .'/'. $nbd_item_key;
                    if( $type != 'png' && $type != 'svg' && $type != 'ibg' && $type != 'png-hires' && $type != 'jpg-hires' && $type != 'pdf' ) $path = $path . '/' .$type;
                    if( $type == 'ibg' || $type == 'png-hires' || $type == 'jpg-hires' || $type == 'pdf' ){
                        $config     = nbd_get_data_from_json($path . '/config.json');
                    ...
                    nbd_zip_files_and_download($zip_files, $pathZip, $nameZip, array(), false);
                    if (file_exists($pathZip)) {
                        $url_zip = Nbdesigner_IO::wp_convert_path_to_url($pathZip);
                        $result['dlf'] = base64_encode($url_zip); // return download file url as base64 endcode
                    } else {
                        $result['dlf'] = '';
                    }
                }
    ...
    public function nbdesigner_detail_order_custom_aws($order_id) {
        // echo "Nguyen Duc Thien";
        $order      = new WC_Order($order_id);
        ...
        $zip_files  = array();
        $files      = array();
        if(isset($download_all) && ($download_all == 'true')){
            $products = $order->get_items();
            foreach($products AS $order_item_id => $product){
                if(wc_get_order_item_meta($order_item_id, '_nbd') || wc_get_order_item_meta($order_item_id, '_nbu')){
                    $nbd_item_key = wc_get_order_item_meta($order_item_id, '_nbd');
    ...
    public function checlLinkDownload($id_order){
        global $wpdb;
	...
	public function nbd_upload_pdf_as_bg_image(){
        ...
        if( $ext == 'pdf' ){
            ...
            if( move_uploaded_file( $file["tmp_name"], $path['full_path'] ) ){
                ...
                if( $res ){
                    $data['flag'] = 1;
                    $data['mes'] = esc_html__( 'Success!', 'web-to-print-online-designer' );1;
                }
            }else{
